...
async fn serve() -> vorma::Result<()> {
	...
	let app = vorma::App::from_config(vorma_notes_example::app_config()?)?;
	let listener = tokio::net::TcpListener::bind(addr)
		.await
		.map_err(|error| vorma::Error::new(format!("bind notes example server: {error}")))?;
		...
		.layer(
			ServiceBuilder::new()
				...
				.layer(vorma::middleware::request_body_limit(
					vorma_notes_example::REQUEST_BODY_LIMIT,
				))
	...
	tracing::info!("Vorma notes example listening on http://{addr}");
	axum::serve(listener, router)
		.with_graceful_shutdown(shutdown_signal())
		.await
		.map_err(|error| vorma::Error::new(format!("serve notes example server: {error}")))
}
