...
async fn serve() -> vorma::Result<()> {
	...
	let app = vorma::App::from_config(vorma_minimal_example::app_config()?)?;
	let listener = tokio::net::TcpListener::bind(addr)
		.await
		.map_err(|error| vorma::Error::runtime(format!("bind minimal example server: {error}")))?;
		...
		.layer(
			ServiceBuilder::new()
				...
				.layer(vorma::middleware::request_body_limit(
					vorma_minimal_example::REQUEST_BODY_LIMIT,
				))
	...
	tracing::info!("Vorma minimal example listening on http://{addr}");
	axum::serve(listener, router)
		.with_graceful_shutdown(shutdown_signal())
		.await
		.map_err(|error| vorma::Error::runtime(format!("serve minimal example server: {error}")))
}
