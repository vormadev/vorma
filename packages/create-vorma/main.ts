...
async function main() {
	...
	if (createNewModule) {
		...
		try {
			...
			if (isLocalTest) {
				...
				execSync(`go mod edit -replace github.com/vormadev/vorma=${vormaPath}`, {
					cwd: moduleRoot,
				});
	...
	const underscoreSegments = pathSegments.filter((seg) => seg.startsWith("_"));
