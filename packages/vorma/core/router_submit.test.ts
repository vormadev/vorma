...
describe("submit", () => {
	...
	it("follows redirect even when response is non-ok", async () => {
		...
		await wait_for(2);
		call(1).resolve(route_response({ matched_patterns: ["/target"] }));
	...
	it("auto-revalidates after settled mutation by default", async () => {
		...
		await wait_for(2);
		call(1).resolve(route_response({ matched_patterns: ["/"] }));
	...
	it("does not re-follow submit redirect to same path", async () => {
		...
		await wait_for(2);
		call(1).resolve(redirect_response({ [X_CLIENT_REDIRECT]: "/current-page" }));
		await sub;
...
describe("submit dedupe edge cases", () => {
	...
	it("stale deduped mutation still triggers revalidation after abort", async () => {
		...
		await wait_for(3);
		call(2).resolve(route_response({ matched_patterns: ["/"] }));
