...
function mock_nav(): LinkNavFns {
	return {
		...
		get_link_state_version: vi.fn().mockReturnValue(0),
		subscribe_link_state: vi.fn().mockReturnValue(() => {}),
...
describe("external detection", () => {
	it("detects cross-origin href as external", () => {
		const nav = mock_nav();
		const result = make_link_props({ href: "https://external.example/page" }, nav);
		expect(result.is_external).toBe(true);
	});

	it("detects mailto scheme as external", () => {
		const nav = mock_nav();
		const result = make_link_props({ href: "mailto:test@example.com" }, nav);
		expect(result.is_external).toBe(true);
	});
...
describe("fallthrough", () => {
	...
	it("does not intercept target=_blank", async () => {
		const nav = mock_nav();
		const result = make_link_props({ href: "/page", target: "_blank" }, nav);
		const ev = primary_click();
...
describe("consumer event forwarding", () => {
	it("forwards onClick to consumer", async () => {
		...
		const nav = mock_nav();
		const result = make_link_props({ href: "/page", onClick: consumer }, nav);
		const ev = primary_click();
	...
	it("forwards onPointerDown to consumer", () => {
		const consumer = vi.fn();
		const nav = mock_nav();
		const result = make_link_props({ href: "/page", onPointerDown: consumer }, nav);
...
describe("prefetch intent", () => {
	it("starts prefetch after delay on pointerenter", () => {
		const nav = mock_nav();
		const result = make_link_props({ href: "/page", prefetch: "intent" }, nav);
...
describe("prefetch none", () => {
	...
	it("does not prefetch on hover when prefetch is none", () => {
		const nav = mock_nav();
		const result = make_link_props({ href: "/page", prefetch: "none" }, nav);
...
describe("prop stripping", () => {
	it("removes vorma keys from anchor props for internal links", () => {
		...
		expect(result.anchor_props).not.toHaveProperty("replace");
		expect(result.anchor_props).not.toHaveProperty("scrollToTop");
		expect(result.anchor_props).not.toHaveProperty(skip_work_indicator_link_prop);
...
describe("route state attributes", () => {
	it("adds Vorma link state attributes from nav state", () => {
		...
		expect(result.anchor_props).toHaveProperty(LINK_ACTIVE_ANCESTOR_ATTR, "");
		expect(result.anchor_props).toHaveProperty(LINK_PENDING_EXACT_ATTR, "");
		expect(result.anchor_props).toHaveProperty(LINK_PENDING_ANCESTOR_ATTR, "");
...
describe("navigate options", () => {
	...
	it("forwards scrollToTop to navigate", async () => {
		const nav = mock_nav();
		const result = make_link_props({ href: "/page", scrollToTop: false }, nav);
	...
	it("forwards skipWorkIndicator to navigate", async () => {
		const nav = mock_nav();
		const result = make_link_props({ href: "/page", skipWorkIndicator: true }, nav);
...
describe("visitOnPointerDown", () => {
	it("navigates on pointerdown for mouse", async () => {
		const nav = mock_nav();
		const result = make_link_props({ href: "/page", visitOnPointerDown: true }, nav);
...
describe("get_href_details", () => {
	...
	it("should return is_http: false for an empty href", () => {
		const result = get_href_details("");
		expect(result).toEqual({ is_http: false });
	});

	it("should treat non-standard strings as relative URLs", () => {
		const result = get_href_details("not-a-valid-url");
	...
	it("should correctly identify internal HTTP URLs", () => {
		const result = get_href_details("https://example.com/page");
		expect(result).toEqual({
			is_http: true,
	...
	it("should correctly identify external HTTP URLs", () => {
		const result = get_href_details("https://external.com");
		expect(result).toEqual({
			is_http: true,
	...
	it("should return is_http: false for non-HTTP protocols", () => {
		const result = get_href_details("mailto:user@example.com");
		expect(result).toEqual({ is_http: false });
	});

	it("should return is_http: false for custom protocols that start with 'http'", () => {
		const result = get_href_details("httpx://example.com/path");
		expect(result).toEqual({ is_http: false });
	});

	it("should correctly resolve relative paths as internal URLs", () => {
		const result = get_href_details("/relative-path");
		expect(result).toEqual({
			is_http: true,
	...
	it("should correctly handle hash links as internal URLs", () => {
		const result = get_href_details("#section");
		expect(result).toEqual({
			is_http: true,
	...
	it("should correctly handle tel: links", () => {
		const result = get_href_details("tel:+1234567890");
		expect(result).toEqual({ is_http: false });
	});
});
