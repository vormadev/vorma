...
export function createVormaClient<A extends AppConfig>(
	...
	function useRouteState<T>(selector?: (route: RouteState) => T): RouteState | T {
	...
	function use_link_state_version(): number {
		return useSyncExternalStore(
			nav_fns.subscribe_link_state,
			nav_fns.get_link_state_version,
			nav_fns.get_link_state_version,
		);
	}
	...
	function usePatternClientLoaderData<T>(pattern: ToViewPattern<A>): T | undefined {
		return use_channel((s) => {
			const idx = s.matched_patterns.indexOf(pattern);
			if (idx < 0) {
				return undefined;
	...
	function BaseLink(
		...
		const link_state_version = use_link_state_version();
		const r = useMemo(() => {
		...
		}, [props, route_state, work_state, link_state_version]);
		return (
			<a
