...
export function createVormaClient<A extends AppConfig>(
...
): VormaClient<A, JSX.Element, JSX.AnchorHTMLAttributes<HTMLAnchorElement>, "accessor"> {
	const [entries, set_entries] = createSignal<DecomposedState["entries"]>([]);
	const [route_error, set_route_error] = createSignal<DecomposedState["error"]>(null);
	const [views_data, set_views_data] = createSignal<unknown[]>([]);
	const [client_loaders_data, set_client_loaders_data] = createSignal<unknown[]>([]);
	...
	const [route_state, set_route_state] = createSignal<RouteState | null>(null, {
		...
		(adapter_commit: DecomposedCommit) => {
			...
			batch(() => {
				if (adapter_commit.state) {
					...
					set_views_data(adapter_commit.state.views_data);
					set_client_loaders_data(adapter_commit.state.client_loaders_data);
				...
				if (adapter_commit.link_state_version !== undefined) {
					set_link_state_version(adapter_commit.link_state_version);
				}
	...
	function useWorkState<T>(selector?: (work: WorkState) => T): Accessor<WorkState | T> {
		return createMemo(
			() => {
	...
	): Accessor<ToViewOutput<A, P>> {
		return createMemo(() => {
			return views_data()[args.idx] as ToViewOutput<A, P>;
		});
	}

	function usePatternViewData<P extends ToViewPattern<A>>(
		pattern: P,
	): Accessor<ToViewOutput<A, P> | undefined> {
		return createMemo(() => {
			...
			return views_data()[idx] as ToViewOutput<A, P>;
		});
	}
	...
	function RootOutlet(props: { idx?: number } & Record<string, unknown>): JSX.Element {
		...
		const error_boundary = createMemo(() => {
			const s = slot();
			return s.kind === "error" ? (s.boundary as ValidComponent) : undefined;
		});
	...
	function BaseLink(
		...
		const anchor_props = createMemo(() => {
			return r().anchor_props as JSX.AnchorHTMLAttributes<HTMLAnchorElement>;
		});
		return (
			<a
