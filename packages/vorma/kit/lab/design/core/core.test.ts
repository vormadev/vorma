...
type ButtonRecipeVariantForTest<TRecipe extends ButtonRecipeForTest> = RecipeVariantValue<
	TRecipe,
	"variant"
>;

function resolve_button_recipe_for_test<const TRecipe extends ButtonRecipeForTest>(
	recipe: TRecipe,
	selection: ButtonRecipeSelectionForTest<TRecipe>,
) {
...
describe("design core", () => {
	it("creates primitive, semantic, and recipe style references", () => {
					...
					compoundVariants: [
						{
							slots: {
								root: {
									base: {
										fontWeight:
											ref.primitive.typography.weight.regular,
									},
									conditions: {
										[hover_condition]: {
											borderColor: ref.semantic.color.surface,
										},
									},
								},
							},
					...
					variants: {
						size: {
							md: {
								root: {
									base: {
										minHeight: ref.primitive.motion.distance.press,
									},
								},
							},
						},
		...
		const recipe_root_base = recipe_system.token.recipe.button.slots.root.base;
		...
		expect(renderCSSVariables(":root", system.modes.light.variables)).toContain(
	...
	it("resolves nested recipe style blocks as opaque style data", () => {
		...
		const recipe_system = createRecipeStyleSystem(system, recipes);
		const item_base = recipe_system.token.recipe.descriptionList.slots.item.base;
	...
	it("requires CSS variable safe prefix and token path segments", () => {
		...
		expect(() => {
			createCSSVariableName("acme", [token_groups.primitive, "camelCase"]);
	...
	it("resolves token mixes", () => {
		...
		expect(
			system.modes.light.variables[
				createCSSVariableName("acme", [token_groups.semantic, "color", "tint"])
			],
		).toBe(
		...
		expect(
			system.modes.light.variables[
				createCSSVariableName("acme", [token_groups.semantic, "layout", "grid"])
			],
		).toBe(
