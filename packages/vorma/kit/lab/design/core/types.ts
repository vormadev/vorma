...
export type ShadowCompositeInput = ShadowLayerInput | readonly ShadowLayerInput[];
...
export type TransitionCompositeInput = TransitionInput | readonly TransitionInput[];
...
export type TokenGroupOutput<T> = T extends TokenSpec
	? string | number
	: {
			[K in keyof T as undefined extends T[K] ? never : K]: TokenGroupOutput<
...
type ResolvedRecipeStyleSlotInput<
	TSlot extends RecipeSlotInput<string, RecipeStyleDeclarationInput>,
> = ResolvedRecipeStyleSlotBase<TSlot> & ResolvedRecipeStyleSlotConditions<TSlot>;
...
> = {
	readonly [K in keyof TSlots]: Exclude<TSlots[K], undefined> extends RecipeSlotInput<
		string,
		RecipeStyleDeclarationInput
	>
...
type ResolvedRecipeStyleVariantsInput<
	TVariants extends RecipeVariantsInput<string, string, RecipeStyleDeclarationInput>,
> = {
	...
	>[]
		? {
				readonly [K in keyof TCompoundVariants]: TCompoundVariants[K] extends RecipeCompoundVariantInput<
					...
					RecipeStyleDeclarationInput
				>
					? ResolvedRecipeStyleCompoundVariantInput<TCompoundVariants[K]>
					: never;
			}
		: never;

export type ResolvedRecipeStyleDefinition<TDefinition extends RecipeStyleDefinition> =
...
export type ResolvedRecipeStyleDefinitions<TDefinitions extends RecipeStyleDefinitions> =
	{
...
export type SystemInput<
	TModes extends NonEmptyReadonlyArray<string> = NonEmptyReadonlyArray<string>,
	TColorSource extends string = string,
	TFontFamily extends string = string,
	TLetterSpacing extends string = string,
	TLineHeight extends string = string,
...
export type ResolvedTokenTree<T> = T extends TokenSpec
	? string | number
	: {
			[K in keyof T as undefined extends T[K] ? never : K]: ResolvedTokenTree<
				Exclude<T[K], undefined>
			>;
		};
