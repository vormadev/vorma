...
export function resolve_token_spec(
	...
	if (spec[token_spec_kind] === token_spec_kinds.ref) {
		return createCSSVariableReference(variable_prefix, spec[token_spec_fields.path]);
	}
	...
	return `color-mix(in ${spec[token_spec_fields.mix].space}, ${resolve_token_spec(
...
export function resolve_token_tree<T>(
	...
	if (is_token_spec(input)) {
		return resolve_token_spec(input, variable_prefix) as ResolvedTokenTree<T>;
	}
