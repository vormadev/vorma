...
function create_test_style_system(): SwitchStyleSystem<"light", typeof switch_recipe> {
	return {
		metadata: {
			breakpoint: {
				md: "48rem",
...
describe("Remix Switch", () => {
	...
	it("renders switch semantics on a native checkbox host", async () => {
		...
		const switch_host = result.$("[data-testid='switch']") as HTMLInputElement | null;
		...
		expect(switch_host.getAttribute(componentAnatomyAttrs.scope)).toBe("switch");
		expect(switch_host.getAttribute(componentAnatomyAttrs.part)).toBe("root");

		await result.act(() => {
			switch_host.checked = true;
			switch_host.dispatchEvent(new Event(checkableChangeEvent, { bubbles: true }));
		});
	...
	it("applies host state and form props to the native input", () => {
		...
		const switch_host = result.$("[data-testid='switch']") as HTMLInputElement;
