...
describe("Remix Progress", () => {
	...
	it("renders accessible single value progress by default", () => {
		...
		expect(
			result.$$("[data-vorma-scope='progress'][data-vorma-part='segment']"),
		).toHaveLength(1);
		result.cleanup();
		...
		const first_segment_style = node.props.children[0][0].props.mix[0].args[0];
	...
	it("supports segmented progress as an extension", () => {
		...
		expect(result.$("[role='progressbar']")?.getAttribute("aria-valuenow")).toBe(
			"55",
		);
		result.cleanup();
		...
		const first_segment_style = node.props.children[0][0].props.mix[0].args[0];
		const second_segment_style = node.props.children[0][1].props.mix[0].args[0];
