...
import { componentAnatomyAttrs, createTabs, type TabsStyleSystem } from "./remix.ts";
...
function create_test_style_system(): TabsStyleSystem<"light", typeof tabs_recipe> {
	return {
		metadata: {},
		modes: {
			light: {
...
describe("Remix Tabs", () => {
	...
	it("selects panels through tab triggers", async () => {
		...
		expect(triggers[0]?.getAttribute(componentStateAttribute)).toBe("selected");
