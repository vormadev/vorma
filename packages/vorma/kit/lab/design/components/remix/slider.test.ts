...
describe("Remix Slider", () => {
	...
	it("applies Slider input props and mix to the range input", async () => {
		...
		expect(input.style.getPropertyValue(sliderProgressVariable)).toBe("25.0000%");
		await result.act(() => {
			input.value = "40";
		...
		expect(input.style.getPropertyValue(sliderProgressVariable)).toBe("40.0000%");
	...
	it("maps Slider pseudo targets onto the input host mix", () => {
		...
		expect(
			(input_style["&::-moz-range-thumb"] as Record<string, unknown>).marginTop,
		).toBeUndefined();
	});
});
