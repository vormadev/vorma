...
describe("Remix Textarea", () => {
	...
	it("renders native textarea semantics and applies props to the textarea host", async () => {
		...
		const result = render(
			createElement(Textarea, {
				...
				mix: on<HTMLTextAreaElement, typeof input_event>(input_event, (event) => {
		...
		expect(textarea.getAttribute(componentAnatomyAttrs.scope)).toBe("textarea");
	...
	it("reflects native textarea state into component data attributes", () => {
		...
		const textarea = result.$("[data-testid='textarea']") as HTMLTextAreaElement;
