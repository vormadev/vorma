...
describe("Remix Checkbox", () => {
	...
	it("renders a native checkbox host with checked state semantics", async () => {
		...
		const checkbox = result.$("[data-testid='checkbox']") as HTMLInputElement | null;
		...
		expect(checkbox.getAttribute(componentAnatomyAttrs.scope)).toBe("checkbox");
		expect(checkbox.getAttribute(componentAnatomyAttrs.part)).toBe("root");

		await result.act(() => {
			checkbox.indeterminate = false;
			checkbox.checked = true;
			checkbox.dispatchEvent(new Event(checkableChangeEvent, { bubbles: true }));
		});
	...
	it("applies host state and form props to the native input", () => {
		...
		const checkbox = result.$("[data-testid='checkbox']") as HTMLInputElement;
