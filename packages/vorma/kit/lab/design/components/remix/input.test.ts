	...
	it("renders native input semantics and applies props to the input host", async () => {
		...
		const result = render(
			createElement(Input, {
				"data-testid": "input",
				htmlSize: 24,
				mix: on<HTMLInputElement, typeof input_event>(input_event, (event) => {
		...
		const input = result.$("[data-testid='input']") as HTMLInputElement | null;
		...
		expect(input.getAttribute(componentAnatomyAttrs.part)).toBe(inputAnatomy.root);
