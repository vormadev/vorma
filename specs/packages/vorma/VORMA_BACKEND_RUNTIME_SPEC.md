# Vorma Backend Runtime Conformance Specification

Status: Draft  
Last Updated: 2026-02-08  
Applies To: Vorma backend runtime behavior as observed through public APIs and
HTTP I/O
...
part of Vorma's observable runtime API behavior.

### BR-ASSET-001: Critical CSS Helper Gating and Element Identity Contract

Given critical CSS entry is not configured or critical CSS artifact is missing  
When critical-css helpers are read (`GetCriticalCSS`,
`GetCriticalCSSStyleElement`, `GetCriticalCSSStyleElementSha256Hash`)  
Then helpers MUST return empty output (no synthesized fallback CSS/style/hash).

Given critical CSS artifact exists  
When helpers are read  
Then:

- `GetCriticalCSS()` MUST return current critical CSS content,
- `GetCriticalCSSStyleElement()` MUST render style element with id
  `wave-critical-css` containing that content,
- `GetCriticalCSSStyleElementSha256Hash()` MUST return CSP-style hash for that
  rendered style element,
- `GetCriticalCSSElementID()` MUST return `wave-critical-css`.

### BR-ASSET-002: Non-Critical Stylesheet URL/Element Contract

Given non-critical CSS entry is not configured  
When non-critical stylesheet helpers are read (`GetStyleSheetURL`,
`GetStyleSheetLinkElement`)  
Then helpers MUST return empty output.

Given non-critical CSS is configured and normal-css ref file resolves filename
`F` under current public-path-prefix `P`  
When helpers are read  
Then:

- `GetStyleSheetURL()` MUST resolve to leading-slash URL for `P + F`,
- `GetStyleSheetLinkElement()` MUST render stylesheet link using that URL and
  id `wave-normal-css`,
- `GetStyleSheetElementID()` MUST return `wave-normal-css`.

### BR-ASSET-003: Public Filemap URL and Runtime Helper Injection Contract

Given public filemap ref resolves filename `M` under current public-path-prefix
`P`  
When `GetPublicFileMapURL()` is read  
Then helper MUST return leading-slash URL for `P + M`.

Given public filemap URL is non-empty  
When `GetPublicFileMapElements()` is read  
Then output MUST include:

- modulepreload link for resolved filemap URL,
- module script that imports `wavePublicFileMap` from that URL,
- assignment of `window.__wave.getPublicURL` helper.

`window.__wave.getPublicURL` behavior contract:

- leading `/` in input path MUST be stripped before map lookup,
- helper MUST return `<PublicPathPrefix> + mapped-dist-name` when key exists in
  `wavePublicFileMap`,
- helper MUST fall back to `<PublicPathPrefix> + original-relative-path` when
  key is absent.

Given filemap URL cannot be resolved  
When `GetPublicFileMapElements()` is read  
Then helper MUST return empty output.

### BR-ASSET-004: Public Filemap Script Hash Contract

Given public filemap elements are available  
When `GetPublicFileMapScriptSha256Hash()` is read  
Then returned hash MUST equal CSP-style hash of generated module-script content
used by `GetPublicFileMapElements()` helper payload.
...
Given fixture variants for missing critical CSS entry, missing critical CSS
artifact, and present critical CSS artifact  
When critical-css helper outputs are sampled  
Then empty-output gating and present-artifact style/hash/id behavior MUST match
contract.

### BRC-ASSET-002 (covers BR-ASSET-002)

Given fixture variants with non-critical CSS entry unset and set  
When stylesheet URL/link/id helpers are sampled  
Then unset case MUST be empty-output, and set case MUST produce
prefix-resolved URL plus stylesheet link element with id `wave-normal-css`.

### BRC-ASSET-003 (covers BR-ASSET-003)

Given fixture variants with resolvable and unresolvable public filemap ref plus
map-hit/map-miss runtime helper inputs  
When filemap URL/elements/helper behavior is observed  
Then URL derivation, element injection payload, and
`window.__wave.getPublicURL` map-hit/map-miss semantics MUST match contract.

### BRC-ASSET-004 (covers BR-ASSET-004)

Given fixture variants with and without resolvable filemap details  
...
- Wire contract: `specs/packages/vorma/VORMA_WIRE_CONTRACT_SPEC.md`
- Frontend runtime:
...
- Interop contracts: `specs/packages/vorma/VORMA_KIT_INTEROP_SPEC.md`
- Testing strategy:
  `specs/packages/vorma/VORMA_TESTING_STRATEGY_SPEC.md`
- Checklist/roadmap: `specs/packages/vorma/VORMA_SPEC_CHECKLIST.md`
