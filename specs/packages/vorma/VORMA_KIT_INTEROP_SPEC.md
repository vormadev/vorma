...
It is not the canonical package spec for dependency internals. Instead, it
specifies interop boundaries where Vorma either:
...
- Canonical owner-internal dependency specs (tracked under
...
Boundary scope fence for this document:

- Vorma-boundary normative sections are `5.1` through `5.5` and `6.1` through
  `6.4`.
- Section `5.6` is owner-reference inventory and is not a Vorma-owner closure
  blocker.
...
- route-collection accessor compatibility:
  `NestedRouter.AllRoutes()` and `Router.AllRoutes()` values exposed through
  `Vorma.LoadersRouter()` / `Vorma.ActionsRouter()` MUST be snapshot-safe for
  callers (caller mutation MUST NOT mutate authoritative route collections used
  by runtime/build flows).
...
- Section `5.6` below is retained as owner-reference inventory and is not a
  Vorma-boundary normative closure gate in this document.

### 5.6 Owner-Reference Utility Inventory (Non-Closure)

The utility/helper packages below are dependency-owner domains.
Vorma tracks them as inherited references only; detailed normative behavior
belongs in owner-package specs and MUST NOT be duplicated here.

Owner-reference inventory:

- `kit/envutil` + `kit/netutil` (environment/default/port helper primitives)
- `kit/reflectutil`
- `kit/middleware`
- `kit/cryptoutil` + `kit/bytesutil`
- `kit/fsutil`
- `kit/executil`
- `kit/id`
- `kit/lru`
- `kit/colorlog`
- `lab/tsgen` + `lab/tsgen/tsgencore`
- `lab/viteutil`
- `lab/jsonschema`
- `lab/stringsutil` + `lab/parseutil`
- `kit/contextutil` + `kit/genericsutil` + `kit/set`
- `kit/grace`
- `lab/esbuildutil`

Conformance ownership rule:

- Vorma conformance at these boundaries is satisfied by owner-reference
  compatibility plus Vorma-observable contracts already defined in
  `VORMA_BACKEND_RUNTIME_SPEC.md`,
  `VORMA_BUILD_DEV_CONFORMANCE_SPEC.md`, and
  `VORMA_PUBLIC_API_SURFACE_SPEC.md`.
- If Vorma starts exposing new direct user-visible guarantees from these
  packages, add explicit boundary requirements there and reference owner IDs
  instead of restating owner internals here.
...
- `vormaclient/client/src/client_loaders.ts`
- `vormaclient/client/src/init_client.ts`
- `vormaclient/client/src/links.ts`
- `vormaclient/client/src/redirects/redirects.ts`
- `vormaclient/client/src/vorma_app_helpers/vorma_app_helpers.ts`
