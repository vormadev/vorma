//! Search-parameter schema derivation from public TypeScript type contracts.

use std::collections::{BTreeMap, BTreeSet};

use serde_json::{Map, Value};

use crate::tsgen::{Type, TypeDef, TypePhase, TypeRef, TypeRegistry};

/// Derive the compact runtime search-parameter schema for a public input type.
...
mod tests {
	use super::*;
	use crate::tsgen::{FieldDef, Result as TsResult};

	const TEST_ALIAS_TYPE_NAME: &str = "KeyAlias";
	const TEST_CYCLE_LEFT_TYPE_NAME: &str = "KeyA";
	const TEST_CYCLE_RIGHT_TYPE_NAME: &str = "KeyB";

	struct MapWithAliasKey;

	impl Type for MapWithAliasKey {
		fn type_ref() -> TypeRef {
			TypeRef::map(TypeRef::named(TEST_ALIAS_TYPE_NAME), TypeRef::String)
		}

		fn collect_type_defs(registry: &mut TypeRegistry) -> TsResult<()> {
			registry.define(TypeDef::alias(TEST_ALIAS_TYPE_NAME, TypeRef::String));
			Ok(())
		}
	}
	...
	impl Type for MapWithCyclicAliasKey {
		fn type_ref() -> TypeRef {
			TypeRef::map(TypeRef::named(TEST_CYCLE_LEFT_TYPE_NAME), TypeRef::String)
		}

		fn collect_type_defs(registry: &mut TypeRegistry) -> TsResult<()> {
			registry.define(TypeDef::alias(
				TEST_CYCLE_LEFT_TYPE_NAME,
				TypeRef::named(TEST_CYCLE_RIGHT_TYPE_NAME),
			));
			registry.define(TypeDef::alias(
				TEST_CYCLE_RIGHT_TYPE_NAME,
				TypeRef::named(TEST_CYCLE_LEFT_TYPE_NAME),
			));
			Ok(())
		}
	}
	...
	impl Type for SearchRecord {
		fn type_ref() -> TypeRef {
			TypeRef::named("SearchRecord")
		}
		...
		fn collect_type_defs(registry: &mut TypeRegistry) -> TsResult<()> {
			registry.define(TypeDef::record(
				"SearchRecord",
				vec![
					FieldDef::required::<String>("q"),
					FieldDef::optional::<i64>("page"),
				],
			));
			Ok(())
		}
	...
	#[test]
	fn record_fields_become_search_param_schema() {
		assert_eq!(
			schema_for_type::<SearchRecord>().unwrap(),
			serde_json::json!({"q": "s", "page": "?n"})
		);
	}
}
