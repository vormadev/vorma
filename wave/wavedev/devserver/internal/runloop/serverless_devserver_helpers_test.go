...
func isConfigFileForRunloopTests(cfg *wave.ParsedConfig, path string) bool {
...
func resolveHookExecutionPlanForRunloopTests(
	...
	if !hook.RunCombinedDevBuildHookCommands || parsedConfig == nil ||
		parsedConfig.FrameworkRunBuildHook == nil {
		return hooks.DeriveHookExecutionPlanFromHook(
			hook,
			func(hookForResolution wave.OnChangeHook) string {
	...
	frameworkBuildHookRunner := parsedConfig.FrameworkRunBuildHook
