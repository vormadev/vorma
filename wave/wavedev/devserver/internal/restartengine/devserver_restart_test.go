...
func TestTriggerRestartWithOpts_UpgradeSemantics(t *testing.T) {
	t.Run(
		"waiting for build retry still upgrades pending restart request",
		func(t *testing.T) {
	...
	t.Run(
		"upgrades pending no-go restart to go-recompile restart",
		func(t *testing.T) {
			harness := newRestartChannelHarnessForRestartTests()
	...
	t.Run("pending config restart is never downgraded", func(t *testing.T) {
		...
		if !request.IsConfigRestart || !request.RecompileGo {
			t.Fatalf(
				"expected config restart to remain pending, got %#v",
				request,
			)
		}
	})
	...
	t.Run(
		"weaker request does not downgrade stronger pending request",
		func(t *testing.T) {
			harness := newRestartChannelHarnessForRestartTests()
...
func TestResolveQueuedRestartRequest(t *testing.T) {
	t.Run(
		"no pending request uses normalized incoming request",
		func(t *testing.T) {
			...
			if !resolvedRequest.RecompileGo ||
				!resolvedRequest.IsConfigRestart {
				t.Fatalf(
					"expected normalized config restart, got %#v",
					resolvedRequest,
				)
			}
		},
...
func expectedMergedRestartRequest(
	...
	return restartengine.RestartRequest{
		RecompileGo: pendingRequest.RecompileGo ||
			incomingRequest.RecompileGo,
		IsConfigRestart: false,
	}
}
...
func TestTriggerRestartFromRefreshActions(t *testing.T) {
	t.Run("trigger restart without go compile", func(t *testing.T) {
		...
		if triggerRestartCalls != 0 {
			t.Fatalf(
				"expected triggerRestartCalls=0, got %d",
				triggerRestartCalls,
			)
		}
	...
	t.Run("trigger restart with go compile", func(t *testing.T) {
		...
		if triggerRestartCalls != 1 {
			t.Fatalf(
				"expected triggerRestartCalls=1, got %d",
				triggerRestartCalls,
			)
		}
