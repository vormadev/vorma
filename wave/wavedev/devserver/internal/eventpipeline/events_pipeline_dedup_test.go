...
func TestDeduplicateWatcherEventsByPath(
	...
	if len(deduplicatedEvents) != 2 {
		t.Fatalf(
			"deduplicated event count = %d, want 2",
			len(deduplicatedEvents),
		)
	}
	if deduplicatedEvents[0].Name != "a.go" {
		t.Fatalf(
			"deduplicated event[0] name = %q, want a.go",
			deduplicatedEvents[0].Name,
		)
	}
	if deduplicatedEvents[1].Name != "b.go" {
		t.Fatalf(
			"deduplicated event[1] name = %q, want b.go",
			deduplicatedEvents[1].Name,
		)
	}
	if !deduplicatedEvents[1].Has(fsnotify.Write) {
		t.Fatalf(
			"deduplicated event[1] op = %v, want write",
			deduplicatedEvents[1].Op,
		)
	}
	if !deduplicatedEvents[1].Has(fsnotify.Create) {
		t.Fatalf(
			"deduplicated event[1] op = %v, want create",
			deduplicatedEvents[1].Op,
		)
	}
}
...
func TestDeduplicateWatcherEventsByPathMergesAllOps(
	...
	if len(deduplicatedEvents) != 1 {
		t.Fatalf(
			"deduplicated event count = %d, want 1",
			len(deduplicatedEvents),
		)
	}
...
func TestDeduplicateWatcherEventsByPath_NormalizesPathShapeVariants(
	...
	if deduplicatedEvents[0].Name != canonicalPath {
		t.Fatalf(
			"deduplicated event name = %q, want %q",
			deduplicatedEvents[0].Name,
			canonicalPath,
		)
	}
	if !deduplicatedEvents[0].Has(fsnotify.Create) ||
		!deduplicatedEvents[0].Has(fsnotify.Write) {
		t.Fatalf(
			"deduplicated op = %v, want create+write",
...
func TestDeduplicateWatcherEventsByPath_MergesSymlinkAliasPaths(
	...
	if !wavecore.PathsReferToSameLocation(
		deduplicatedEvents[0].Name,
		targetFilePath,
	) {
		t.Fatalf(
...
func TestDeduplicateWatcherEventsByPath_MergesMissingFileAliasPaths(
	...
	deduplicatedAliasKey := dedup.MissingFileAliasKeyForWatcherEventDeduplication(
		deduplicatedEvents[0].Name,
	)
	targetAliasKey := dedup.MissingFileAliasKeyForWatcherEventDeduplication(
		targetMissingFilePath,
	)
	if deduplicatedAliasKey == "" || targetAliasKey == "" ||
		deduplicatedAliasKey != targetAliasKey {
		t.Fatalf(
...
func TestDeduplicateWatcherEventsByPath_PrefersFirstSeenAliasPathKey(
	...
	testCases := []struct {
	...
	}{
		{
			...
			ExpectedPathKey:  wavecore.Absolute(aliasFilePath),
		...
		},
		{
			...
			ExpectedPathKey:  wavecore.Absolute(targetFilePath),
			ExpectedEventOps: fsnotify.Create | fsnotify.Write,
		},
	}
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			deduplicatedEvents := dedup.DeduplicateWatcherEventsByPath(
				testCase.Events,
			)
			if len(deduplicatedEvents) != 1 {
				t.Fatalf(
...
func TestWatcherEventDeduplicationIndex_MemoizesCanonicalAndAliasResolution(
	...
	absoluteAliasFilePath := wavecore.Absolute(aliasFilePath)
	absoluteAliasMissingFilePath := wavecore.Absolute(aliasMissingFilePath)

	canonicalPathFirst := index.ResolveCanonicalPathForAbsolutePath(
		absoluteAliasFilePath,
	)
	canonicalPathSecond := index.ResolveCanonicalPathForAbsolutePath(
		absoluteAliasFilePath,
	)
	if canonicalPathFirst == "" || canonicalPathSecond == "" ||
		canonicalPathFirst != canonicalPathSecond {
		t.Fatalf(
	...
	aliasKeyFirst := index.ResolveMissingAliasKeyForAbsolutePath(
		absoluteAliasMissingFilePath,
	)
	aliasKeySecond := index.ResolveMissingAliasKeyForAbsolutePath(
		absoluteAliasMissingFilePath,
	)
	if aliasKeyFirst == "" || aliasKeySecond == "" ||
		aliasKeyFirst != aliasKeySecond {
		t.Fatalf(
	...
	rootAliasKey := index.ResolveMissingAliasKeyForAbsolutePath(
...
func TestWatcherEventDeduplicationIndex_ProbeResolutionMissesKeepCachesStable(
	...
	if existingPathKey := index.ResolveExistingPathKey(
		mergedEventOpsByPath,
		"relative/file.css",
	); existingPathKey != "" {
		t.Fatalf(
			"expected no existing path key for relative probe path, got %q",
			existingPathKey,
		)
	}
	if canonicalPath := index.ResolveCanonicalPathForAbsolutePath(
		"relative/file.css",
	); canonicalPath != "" {
		t.Fatalf(
			"expected no canonical path for relative probe path, got %q",
			canonicalPath,
		)
	}
	if missingAliasKey := index.ResolveMissingAliasKeyForAbsolutePath(
		"relative/missing.css",
	); missingAliasKey != "" {
		t.Fatalf(
			"expected no missing alias key for relative probe path, got %q",
			missingAliasKey,
		)
	}
	...
	if len(index.CanonicalPathByAbsolutePath) != 0 {
		t.Fatalf(
			"expected canonical path cache to remain empty on misses, got %#v",
			index.CanonicalPathByAbsolutePath,
		)
	}
	if len(index.MissingAliasKeyByAbsolutePath) != 0 {
		t.Fatalf(
			"expected missing alias cache to remain empty on misses, got %#v",
			index.MissingAliasKeyByAbsolutePath,
		)
	}
	if len(index.CanonicalPathToPathKey) != 0 {
		t.Fatalf(
			"expected canonical path-key map to remain empty on misses, got %#v",
			index.CanonicalPathToPathKey,
		)
	}
	if len(index.MissingFileAliasKeyToPathKey) != 0 {
		t.Fatalf(
			"expected missing alias path-key map to remain empty on misses, got %#v",
...
func TestWatcherEventDeduplicationIndex_RecordPathKeyPreservesFirstSeenProbePathKey(
	...
	firstCanonicalAliasPath := wavecore.Absolute(
		filepath.Join(aliasDirectoryPathA, "styles.css"),
	)
	secondCanonicalAliasPath := wavecore.Absolute(
		filepath.Join(aliasDirectoryPathB, "styles.css"),
	)
	index.RecordPathKey(firstCanonicalAliasPath)
	index.RecordPathKey(secondCanonicalAliasPath)

	canonicalProbeKey := index.ResolveCanonicalPathForAbsolutePath(
		secondCanonicalAliasPath,
	)
	...
	firstMissingAliasPath := wavecore.Absolute(
		filepath.Join(aliasDirectoryPathA, "missing.css"),
	)
	secondMissingAliasPath := wavecore.Absolute(
		filepath.Join(aliasDirectoryPathB, "missing.css"),
	)
	index.RecordPathKey(firstMissingAliasPath)
	index.RecordPathKey(secondMissingAliasPath)

	missingAliasProbeKey := index.ResolveMissingAliasKeyForAbsolutePath(
		secondMissingAliasPath,
	)
...
func TestNormalizeWatcherEventPathForDeduplication_PathShapes(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			ExpectedPath: wavecore.Absolute(
				filepath.Join(root, "backend", "main.go"),
			),
		},
	}
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			normalizedPath := dedup.NormalizeWatcherEventPathForDeduplication(
				testCase.RawPath,
			)
...
func TestNormalizeHookContextPathShape_PathShapes(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			RawPath:      "  ./backend/./main.go  ",
			ExpectedPath: wavecore.Absolute("./backend/main.go"),
		},
		{
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			normalizedPath := hooks.NormalizeHookContextPathShape(
				testCase.RawPath,
			)
			if normalizedPath != testCase.ExpectedPath {
				t.Fatalf(
					"hooks.NormalizeHookContextPathShape(%q) = %q, want %q",
					testCase.RawPath,
					normalizedPath,
					testCase.ExpectedPath,
				)
	...
	for _, seed := range seeds {
		...
		t.Run("seed_"+strconv.FormatInt(seedValue, 10), func(t *testing.T) {
			...
			if !sort.SliceIsSorted(deduplicatedEvents, func(i int, j int) bool {
				...
				t.Fatalf(
					"expected deduplicated events to be sorted by path, got %#v",
					deduplicatedEvents,
				)
			}

			idempotentDeduplicatedEvents := dedup.DeduplicateWatcherEventsByPath(
				deduplicatedEvents,
			)
			...
			for _, originalEvent := range events {
				...
				for _, deduplicatedEvent := range deduplicatedEvents {
					if !pathsEquivalentForDedupContract(
						originalEvent.Name,
						deduplicatedEvent.Name,
					) {
						continue
					}
...
func pathsEquivalentForDedupContract(pathA string, pathB string) bool {
	...
	if filepath.IsAbs(normalizedPathA) &&
		...
		wavecore.PathsReferToSameLocation(normalizedPathA, normalizedPathB) {
		return true
	}

	if filepath.IsAbs(normalizedPathA) && filepath.IsAbs(normalizedPathB) {
		aliasKeyA := dedup.MissingFileAliasKeyForWatcherEventDeduplication(
			normalizedPathA,
		)
		aliasKeyB := dedup.MissingFileAliasKeyForWatcherEventDeduplication(
			normalizedPathB,
		)
...
func TestClassifyWatcherEventsForProcessingDoesNotCollapseImplicitFileTypes(
	...
	if err := os.MkdirAll(filepath.Dir(publicStaticFilePath), 0o755); err != nil {
	...
	if err := os.WriteFile(publicStaticFilePath, []byte("logo"), 0o644); err != nil {
	...
	for _, classifiedEvent := range classifiedEvents {
		if classifiedEvent.FileType == eventpipeline.FileTypeGo {
			foundGoEvent = true
		}
		if classifiedEvent.FileType == eventpipeline.FileTypePublicStatic {
...
func TestClassifyWatcherEventsForProcessingPreservesSharedPatternEvents(
	...
	if err := os.WriteFile(textFilePath, []byte("notes"), 0o644); err != nil {
...
func TestBuildEventHooksForProcessingDeduplicatesHooksByPattern(
	...
	if !eventsWithHooks[1].SkipDuplicateHooks {
		t.Fatal(
			"expected second event with shared pattern to skip duplicate hooks",
		)
	}
	if !eventpipeline.AnyEventNeedsHardReload(eventsWithHooks) {
		t.Fatal(
			"expected batchNeedsAppStop=true when one event requires hard reload",
		)
	}
	if got, want := eventsWithHooks[0].HookCtx.ChangedFilePaths, []string{
		wavecore.Absolute("a.go"),
		wavecore.Absolute("b.txt"),
	}; !reflect.DeepEqual(got, want) {
		t.Fatalf(
			"first hook context changed file paths = %v, want %v",
			got,
			want,
		)
	}
	if got, want := eventsWithHooks[1].HookCtx.ChangedFilePaths, []string{
		...
		t.Fatalf(
			"second hook context changed file paths = %v, want %v",
			got,
			want,
		)
	}
...
func TestBuildEventHooksForProcessing_NormalizesHookContextPathsByPathShape(
	...
	if got := eventsWithHooks[0].HookCtx.FilePath; got != normalizedPath {
		t.Fatalf(
			"first hook context file path = %q, want %q",
			got,
			normalizedPath,
		)
	}
	if got := eventsWithHooks[1].HookCtx.FilePath; got != normalizedPath {
		t.Fatalf(
			"second hook context file path = %q, want %q",
			got,
			normalizedPath,
		)
	}
	...
	if got := eventsWithHooks[0].HookCtx.ChangedFilePaths; !reflect.DeepEqual(
		got,
	...
	if got := eventsWithHooks[1].HookCtx.ChangedFilePaths; !reflect.DeepEqual(
		got,
		expectedChangedPaths,
	) {
		t.Fatalf(
...
func TestBuildNormalizedChangedFilePathsByWatchedPatternForHookContexts(
	...
	patternAFirstPathWithDotSegment := filepath.Join(
		root,
		"pattern-a",
		".",
		"one.txt",
	...
	classifiedEvents := []eventpipeline.ClassifiedEvent{
		{
			Event: fsnotify.Event{
				Name: patternAFirstPathWithDotSegment,
				Op:   fsnotify.Write,
			},
			WatchedFile: &wave.WatchedFile{
				Pattern: "**/*.txt",
		...
		{
			Event: fsnotify.Event{
				Name: patternAFirstNormalizedPath,
				Op:   fsnotify.Write,
			},
			WatchedFile: &wave.WatchedFile{
				Pattern: "**/*.txt",
		...
		{
			Event: fsnotify.Event{
				Name: patternASecondNormalizedPath,
				Op:   fsnotify.Write,
			},
			WatchedFile: &wave.WatchedFile{
				Pattern: "**/*.txt",
		...
		{
			Event: fsnotify.Event{
				Name: filepath.Join(root, "no-pattern.txt"),
				Op:   fsnotify.Write,
			},
			WatchedFile: nil,
		},
	...
	normalizedChangedFilePathsByWatchedPattern := hooks.BuildNormalizedChangedFilePathsByWatchedPatternForHookContexts(
		classifiedEvents,
	)
	...
	if got := normalizedChangedFilePathsByWatchedPattern["**/*.txt"]; !reflect.DeepEqual(
		got,
		expectedPatternATrackedPaths,
	) {
		t.Fatalf(
			"normalized changed paths for **/*.txt = %v, want %v",
			got,
			expectedPatternATrackedPaths,
		)
	}
	...
	if got := normalizedChangedFilePathsByWatchedPattern["**/*.md"]; !reflect.DeepEqual(
		got,
		expectedPatternBTrackedPaths,
	) {
		t.Fatalf(
			"normalized changed paths for **/*.md = %v, want %v",
			got,
			expectedPatternBTrackedPaths,
		)
	}
...
func TestBuildSkipDuplicateHooksByClassifiedEventIndex(t *testing.T) {
	...
	skipDuplicateHooksByClassifiedEventIndex := hooks.BuildSkipDuplicateHooksByClassifiedEventIndex(
		classifiedEvents,
	)
...
func TestBuildEventHooksForProcessingNoPatternUsesSingleChangedFilePath(
	...
	if got, want := eventsWithHooks[0].HookCtx.ChangedFilePaths, []string{"/tmp/standalone.go"}; !reflect.DeepEqual(
		got,
		want,
	) {
...
func TestProcessEvents_DeduplicatesHooksByPatternForMixedFileTypes(
	...
	if got := atomic.LoadInt32(&callbackCount); got != 1 {
		t.Fatalf(
			"expected callback to run once for shared pattern across mixed file types, got %d",
			got,
		)
	}
}
...
func TestProcessEvents_DeduplicatesHooksByPatternAcrossAllHookStages(
	...
	if got := atomic.LoadInt32(&preHookCount); got != 1 {
		t.Fatalf(
			"expected pre hook to run once for shared pattern, got %d",
			got,
		)
	}
	if got := atomic.LoadInt32(&concurrentHookCount); got != 1 {
		t.Fatalf(
			"expected concurrent hook to run once for shared pattern, got %d",
			got,
		)
	}
	if got := atomic.LoadInt32(&postHookCount); got != 1 {
		t.Fatalf(
			"expected post hook to run once for shared pattern, got %d",
			got,
		)
	}
	if got := atomic.LoadInt32(&noWaitHookCount); got != 1 {
		t.Fatalf(
			"expected concurrent-no-wait hook to run once for shared pattern, got %d",
			got,
		)
	}
}
