...
func TestWriteFileAtomicWithDependencies_RetriesAfterExistingTarget(
	t *testing.T,
) {
	root := t.TempDir()
	...
	if writeError != nil {
		t.Fatalf(
			"WriteFileAtomicWithDependencies returned error: %v",
			writeError,
		)
	}
	if renameCallCount < 2 {
		t.Fatalf(
			"expected rename to be attempted at least twice, got %d",
			renameCallCount,
		)
	}
