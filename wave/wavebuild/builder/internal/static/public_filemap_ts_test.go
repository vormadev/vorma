...
func TestWritePublicFileMapTS_WritesSortedTSAndJSONOutputs(t *testing.T) {
	...
	if !strings.Contains(tsContent, "\"a.js\": \"vorma_out_a_deadbeef.js\"") {
		t.Fatalf(
			"expected TS output to contain a.js entry, got:\n%s",
			tsContent,
		)
	}
	if !strings.Contains(tsContent, "\"z.js\": \"vorma_out_z_deadbeef.js\"") {
		t.Fatalf(
			"expected TS output to contain z.js entry, got:\n%s",
			tsContent,
		)
	}
	if strings.Index(
	...
	if _, statErr := os.Stat(jsonPath); statErr != nil {
		t.Fatalf(
			"expected JSON file to be generated at %s: %v",
			jsonPath,
			statErr,
		)
	}
...
func TestWriteFrameworkPublicFileMapTS_ServerOnlyModeWithoutFileMap(
	t *testing.T,
) {
	root := t.TempDir()
	outDir := filepath.Join(root, "generated")
	...
	if !strings.Contains(string(tsData), "staticPublicAssetMap = {") {
		t.Fatalf(
			"expected generated TS output to include staticPublicAssetMap, got:\n%s",
			string(tsData),
		)
	}
}
