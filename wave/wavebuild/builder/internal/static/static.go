...
import (
	...
	"github.com/vormadev/vorma/wave/wavebuild/builder/internal/fileops"
)
...
func (processor *Processor) WriteFrameworkPublicFileMapTS() error {
	...
	if writeError := shared.WriteFileAtomically(outputPath, []byte(jsContent), 0o644); writeError != nil {
		return writeError
	}
	...
	if frameworkDirectory := strings.TrimSpace(processor.cfg.FrameworkPublicFileMapOutDir); frameworkDirectory != "" {
		if writeFrameworkError := writeFrameworkPublicFileMapOutputs(
			frameworkDirectory,
			publicFileMap,
		); writeFrameworkError != nil {
...
func writeFrameworkPublicFileMapOutputs(
	frameworkDirectory string,
	publicFileMap wave.FileMap,
) error {
	if mkdirError := os.MkdirAll(frameworkDirectory, 0o755); mkdirError != nil {
		return mkdirError
	}
	...
	for _, key := range sortedKeys {
		typeScriptBuilder.WriteString(
			fmt.Sprintf("\t%q: %q,\n", key, publicFileMap[key].DistName),
		)
	}
	typeScriptBuilder.WriteString("} as const;\n")

	typeScriptOutputPath := filepath.Join(
		frameworkDirectory,
	...
	fileMapJSON := make(map[string]string, len(publicFileMap))
	for _, key := range sortedKeys {
		fileMapJSON[key] = publicFileMap[key].DistName
	}
	serializedJSON, marshalError := json.MarshalIndent(
		fileMapJSON,
		"",
		"\t",
	)
	...
	jsonOutputPath := filepath.Join(
		frameworkDirectory,
		wave.RelPaths.PublicFileMapJSONName(),
	)
	return shared.WriteFileAtomically(
		jsonOutputPath,
		serializedJSON,
		0o644,
	)
...
// SavePublicFileMapJS writes hashed JS module + ref for public file map.
func (processor *Processor) SavePublicFileMapJS(fileMap wave.FileMap) error {
	if processor == nil || processor.cfg == nil {
		return errors.New("static processor config is nil")
	}
	...
	serializedFileMap, marshalError := json.Marshal(simpleMap)
	if marshalError != nil {
		return marshalError
	}
	content := fmt.Sprintf(
...
// WritePublicFileMapTS writes TS and JSON public file maps to outDir.
func (processor *Processor) WritePublicFileMapTS(outDir string) error {
	fileMap, loadError := processor.loadOrBuildFileMap()
	if loadError != nil {
		return fmt.Errorf("load file map: %w", loadError)
	}
	...
	for _, key := range keys {
		typeScriptBuilder.WriteString(
			fmt.Sprintf("\t%q: %q,\n", key, fileMap[key].DistName),
		)
	}
	typeScriptBuilder.WriteString("} as const;\n")

	if mkdirError := os.MkdirAll(outDir, 0o755); mkdirError != nil {
	...
	if _, writeError := fileops.WriteFileAtomicBytesIfChanged(
		...
		return fmt.Errorf("write TS file: %w", writeError)
	}
	...
	serializedJSON, marshalError := json.MarshalIndent(simpleMap, "", "\t")
	if marshalError != nil {
...
func (processor *Processor) loadOrBuildFileMap() (wave.FileMap, error) {
	if processor == nil || processor.cfg == nil {
...
func (processor *Processor) LoadPublicFileMap() (wave.FileMap, error) {
	if processor == nil || processor.cfg == nil {
		return nil, errors.New("static processor config is nil")
	}
...
// AddPublicAssetKeys appends typed public asset key definitions.
func (processor *Processor) AddPublicAssetKeys(
	...
	keys, keysError := processor.PublicFileMapKeys()
	if keysError != nil {
		return nil, fmt.Errorf("public file map keys: %w", keysError)
	}
...
func (processor *Processor) processFullScan(
	...
	if _, statError := os.Stat(store.sourceRootPath); os.IsNotExist(statError) {
		...
		emptyFileMap := make(wave.FileMap)
...
func (processor *Processor) currentPublicFileMap() (wave.FileMap, error) {
	...
	if loadError != nil {
		if !processor.cfg.UsingBrowser() &&
			...
			processor.cachedPublicFileMap = make(wave.FileMap)
			return cloneFileMap(processor.cachedPublicFileMap), nil
		}
		return nil, loadError
	}
...
func copySourceFileToDistPath(
	...
	if ensureDirectoryError := shared.EnsureDirectoryForFile(destinationPath); ensureDirectoryError != nil {
		return ensureDirectoryError
	}
	return shared.CopyFileAtomically(sourcePath, destinationPath)
}
...
func saveFileMap(path string, fileMap wave.FileMap) error {
	...
	if ensureDirectoryError := shared.EnsureDirectoryForFile(path); ensureDirectoryError != nil {
		return ensureDirectoryError
	}
	return fileops.WriteFileAtomic(path, func(file *os.File) error {
		encoder := gob.NewEncoder(file)
		return encoder.Encode(fileMap)
	})
}
...
func ensureNoStaticLogicalPathCollision(
	...
	existingPath := wavecore.Absolute(existingFileInfo.srcPath)
	candidatePath := wavecore.Absolute(candidateFileInfo.srcPath)
	if wavecore.PathsReferToSameLocation(existingPath, candidatePath) {
		return nil
	}
	...
	for _, candidateFileInfo := range candidateFileInfos {
		...
		if sourceExists {
			existingSourcePaths = append(
				existingSourcePaths,
				wavecore.Absolute(candidateFileInfo.srcPath),
			)
		}
	}
...
func resolveStaticChangedPathResolutions(
	...
	return resolveStaticChangedPathResolutionsWithProbeFunctions(
		...
		changedSourcePaths,
		staticChangedPathResolutionProbeFunctions{
			normalizeChangedSourcePath: wavecore.CanonicalizePathForLocationComparison,
			resolveStaticFileInfoFromSourcePath: func(
				sourceDirectoryPath string,
				sourcePath string,
			) (fileInfo, bool, error) {
