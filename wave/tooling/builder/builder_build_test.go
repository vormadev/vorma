...
func TestProcessFiles_FullCleanupPreservesOnlyWaveDevLockFile(t *testing.T) {
	...
	if _, statLockError := os.Stat(lockFilePath); statLockError != nil {
		t.Fatalf("expected lock file to be preserved, stat error = %v", statLockError)
	}
	if _, statNonLockWavePrefixedFileError := os.Stat(nonLockWavePrefixedFilePath); !os.IsNotExist(statNonLockWavePrefixedFileError) {
		t.Fatalf(
			"expected non-lock .wave-* file to be removed, stat error = %v",
			statNonLockWavePrefixedFileError,
		)
	}
	if _, statRegularFileError := os.Stat(regularFilePath); !os.IsNotExist(statRegularFileError) {
		t.Fatalf(
...
func TestWritePublicFileMapTS_WritesTypeScriptAndJSON(t *testing.T) {
	...
	publicAssetPath := filepath.Join(config.Core.StaticAssetDirs.Public, "assets", "logo.txt")
	...
	if _, statError := os.Stat(typeScriptPath); statError != nil {
		t.Fatalf("expected TS file at %q, stat error: %v", typeScriptPath, statError)
	}
	...
	if !strings.Contains(string(jsonBytes), "assets/logo.txt") {
		t.Fatalf("expected JSON map to include source path key, got %q", string(jsonBytes))
	}
}
...
func TestSavePublicFileMapJS_WritesHashedArtifactAndRef(t *testing.T) {
	...
	if hashedArtifactName == "" {
		t.Fatal("expected public file map ref file to contain hashed artifact filename")
	}

	hashedArtifactPath := filepath.Join(config.Dist.StaticPublic(), hashedArtifactName)
	if _, statError := os.Stat(hashedArtifactPath); statError != nil {
		t.Fatalf("expected hashed public file map artifact at %q: %v", hashedArtifactPath, statError)
	}
}
