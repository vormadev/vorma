...
func TestRunBuildHooks_FailFastOnUserHookError(t *testing.T) {
	...
	if _, statError := os.Stat(outPath); !os.IsNotExist(statError) {
		t.Fatalf("expected framework hook not to run, stat error: %v", statError)
	}
}
...
func TestRunBuildHooks_ReportsFrameworkHookErrorAfterUserHookRuns(t *testing.T) {
	...
	content, readError := os.ReadFile(outPath)
	if readError != nil {
		t.Fatalf("expected user hook output to be written before framework failure: %v", readError)
	}
	if string(content) != "user\n" {
...
func TestRunBuildHooks_DevHookTimeoutStopsUserHookQuickly(t *testing.T) {
	...
	if _, statError := os.Stat(frameworkMarkerPath); !os.IsNotExist(statError) {
		t.Fatalf("expected framework hook not to run after timed-out dev hook, stat error: %v", statError)
	}
}
