...
func TestCanonicalizePathForLocationComparison_FollowsParentSymlinkForMissingLeaf(
	...
	canonicalizedPath := wavecore.CanonicalizePathForLocationComparison(missingLeafUnderAlias)
	if !wavecore.PathsReferToSameLocation(canonicalizedPath, missingLeafUnderTarget) {
		t.Fatalf(
...
func TestProcessPrivateFilesOnly_PreservesRelativePaths(t *testing.T) {
	...
	privateMap, err := loadFileMapFromPathForStaticProcessingTests(cfg, cfg.Dist.PrivateFileMapGob())
	if err != nil {
...
func TestProcessPrivateFilesOnlyForChangedPaths_RemovesDeletedEntry(t *testing.T) {
	...
	initialMap, err := loadFileMapFromPathForStaticProcessingTests(cfg, cfg.Dist.PrivateFileMapGob())
	if err != nil {
	...
	updatedMap, err := loadFileMapFromPathForStaticProcessingTests(cfg, cfg.Dist.PrivateFileMapGob())
	if err != nil {
...
func newParsedConfigForStaticProcessingTestsAtRoot(root string) *wave.ParsedConfig {
