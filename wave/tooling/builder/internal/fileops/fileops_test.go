...
import (
	...
	"github.com/vormadev/vorma/wave/tooling/builder/internal/fileops"
)
...
func TestHashFile_MatchesHashBytesForSameInputs(t *testing.T) {
	...
	if !strings.HasPrefix(hashedFromFile, wave.HashedOutputPrefix) {
		t.Fatalf(
...
func TestComputeFileContentHash_Deterministic(t *testing.T) {
	...
	if firstHash != secondHash {
		t.Fatalf("expected deterministic hash, first=%q second=%q", firstHash, secondHash)
	}
}
...
func TestPublishHashedArtifactWithRef_ReplacesPreviousArtifact(t *testing.T) {
	...
	if writtenName != nextArtifactName {
		t.Fatalf("written artifact name = %q, want %q", writtenName, nextArtifactName)
	}
	if _, statPreviousError := os.Stat(previousArtifactPath); !os.IsNotExist(statPreviousError) {
		t.Fatalf("expected previous artifact to be removed, stat error: %v", statPreviousError)
	}
	...
	if string(nextBytes) != "new" {
		t.Fatalf("next artifact content = %q, want %q", string(nextBytes), "new")
	}
...
func TestPublishHashedArtifactWithRef_NoOpWhenRefAndArtifactAlreadyMatch(t *testing.T) {
	root := t.TempDir()
	outputDir := filepath.Join(root, "public")
	refPath := filepath.Join(root, "public-filemap.ref")
	if mkdirError := os.MkdirAll(outputDir, 0o755); mkdirError != nil {
	...
	if writtenName != artifactName {
		t.Fatalf("written artifact name = %q, want %q", writtenName, artifactName)
	}
	...
	if string(contentBytes) != "existing" {
		t.Fatalf("expected existing artifact content to remain unchanged, got %q", string(contentBytes))
	}
}

func TestWriteFileAtomicWithDependencies_RetriesAfterExistingTarget(t *testing.T) {
	root := t.TempDir()
	targetPath := filepath.Join(root, "target.txt")
	if writeError := os.WriteFile(targetPath, []byte("old"), 0o644); writeError != nil {
		t.Fatalf("write initial target: %v", writeError)
	}

	var renameCallCount int32
	writeError := fileops.WriteFileAtomicWithDependencies(
		targetPath,
		func(file *os.File) error {
			_, innerWriteError := file.Write([]byte("new"))
			return innerWriteError
		},
		fileops.AtomicFileWriteDependencies{
			RenameTempFile: func(sourcePath string, destinationPath string) error {
				renameAttemptCount := atomic.AddInt32(&renameCallCount, 1)
	...
	if writeError != nil {
		t.Fatalf("WriteFileAtomicWithDependencies returned error: %v", writeError)
	}
	if renameCallCount < 2 {
		t.Fatalf("expected rename to be attempted at least twice, got %d", renameCallCount)
	}

	targetBytes, readError := os.ReadFile(targetPath)
	if readError != nil {
		t.Fatalf("read target after write: %v", readError)
	}
	if string(targetBytes) != "new" {
		t.Fatalf("target content = %q, want %q", string(targetBytes), "new")
	}
}
