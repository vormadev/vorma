...
func PublishHashedArtifactWithRef(
	...
	if hasValidRef && previousArtifactFileName == options.DesiredHashedFileName {
	...
	if hasValidRef && previousArtifactFileName != options.DesiredHashedFileName {
		removeHashedArtifactIfPresent(
			options.Log,
			filepath.Join(options.OutputDirectoryPath, previousArtifactFileName),
		)
	}
	if !hasValidRef {
...
func shouldRetryRenameByReplacingTarget(
	...
	if errors.Is(renameError, fs.ErrExist) || errors.Is(renameError, os.ErrExist) {
		return true
	}
