...
func TestInitWatcher_AddsConfigFileDirectoryOutsideWatchRoot(t *testing.T) {
	...
	outsideConfigFilePath := filepath.Join(outsideConfigDirectory, "wave.config.json")
...
func TestWaitForVite_UsesViteClientEndpoint(t *testing.T) {
	...
	s := &Server{
		Log:         newDiscardLogger(),
		ViteContext: vitecmd.NewBuildCtx(&vitecmd.BuildCtxOptions{DefaultPort: port}),
	}
...
func TestStartViteAndStopVite_NoOpWhenViteDisabled(t *testing.T) {
	...
	if s.ViteContext != nil {
		t.Fatalf("expected no vite context when Vite is disabled, got %#v", s.ViteContext)
	}
...
func TestCycleVite_StartFailureAfterStopLeavesViteContextCleared(t *testing.T) {
	...
	s := &Server{
		...
		Log:         newDiscardLogger(),
		Builder:     builder,
		ViteContext: vitecmd.NewBuildCtx(&vitecmd.BuildCtxOptions{DefaultPort: cfg.Vite.DefaultPort}),
	}
