...
func setupSiteRegressionHarnessForToolingTestsWithOptions(
	...
	parsedConfig, parseError := wave.ParseConfigFile(paths.ConfigFilePath)
	if parseError != nil {
		t.Fatalf("parse config payload: %v", parseError)
	}
	frameworkReloadActionCallback := func(*wave.HookContext) (*wave.RefreshAction, error) {
	...
	parsedConfig.FrameworkWatchPatterns = []wave.WatchedFile{
		{
	...
	for watchedFileIndex := range parsedConfig.FrameworkWatchPatterns {
		parsedConfig.FrameworkWatchPatterns[watchedFileIndex].Sort()
	}
...
func assertExpectedBroadcastPayloadForSiteRegressionHarness(
	...
	if readError := connection.ReadJSON(&receivedPayload); readError != nil {
		t.Fatalf("%s: expected broadcast payload: %v", contextMessage, readError)
	}
...
func TestSiteRegression_NormalCSSEditHotReloadsWhenWatchRootIsAncestor(
	...
	if receivedPayload.NormalCSSURL == "" {
		t.Fatalf("expected non-empty normal css URL payload, got %#v", receivedPayload)
	}
}
...
func TestSiteRegression_ConfigSemanticWriteBroadcastsRebuildingAndQueuesConfigRestart(
	...
	writeSemanticallyChangedConfigForToolingTests(t, harness.Paths.ConfigFilePath)
