...
func setupConfigEventTestConfigForEventPlanTests(t *testing.T) (*wave.ParsedConfig, string, string) {
	t.Helper()
...
func TestBuildEventExecutionPlan_ConfigChangeHasNoPlan(t *testing.T) {
	runConfigMutationAndPathShapeMatrixForEventPlanTests(
		...
		func(
			...
			cfg, _, configFilePath := setupConfigEventTestConfigForEventPlanTests(t)
...
func TestBuildEventExecutionPlan_BatchPlanIncludesHookBatchContext(t *testing.T) {
	...
	watcherForTest, err := watch.NewWatcher(cfg, newDiscardLoggerForEventPipelineDedupTests())
	if err != nil {
		t.Fatalf("watch.NewWatcher returned error: %v", err)
	}
	defer watcherForTest.Close()

	builderForTest := builder.NewBuilder(cfg, newDiscardLoggerForEventPipelineDedupTests())
	defer builderForTest.Close()
...
func TestResolveAppStopStrategy(t *testing.T) {
	t.Run("returns none for empty plan", func(t *testing.T) {
		if got := eventpipeline.ResolveAppStopStrategy(nil); got != eventpipeline.AppStopStrategyNone {
			t.Fatalf("eventpipeline.ResolveAppStopStrategy(nil)=%v, want %v", got, eventpipeline.AppStopStrategyNone)
		}
	})
	...
	t.Run("batch without hard reload uses none strategy", func(t *testing.T) {
		...
		if got := eventpipeline.ResolveAppStopStrategy(eventsWithHooks); got != eventpipeline.AppStopStrategyNone {
			t.Fatalf("eventpipeline.ResolveAppStopStrategy(batch no hard reload)=%v, want %v", got, eventpipeline.AppStopStrategyNone)
		}
	})
}
...
func TestBuildEventExecutionPlanFromClassifiedEvents_HookContextFilePathIsAbsolute(
	...
	if len(eventsWithHooks) != 1 {
		t.Fatalf("expected one eventpipeline.EventWithHooks entry, got %#v", eventsWithHooks)
	}

	gotHookContextFilePath := eventsWithHooks[0].HookCtx.FilePath
	wantHookContextFilePath := wavecore.Absolute(relativeChangedPath)
...
func TestDeriveEventExecutionPlanBehavioralDecisionFromEventsWithHooks(t *testing.T) {
	t.Run("empty inputs produce zero behavioral decision", func(t *testing.T) {
		decision := eventpipeline.DeriveEventExecutionPlanBehavioralDecisionFromEventsWithHooks(nil)
	...
	t.Run("css-only classified events suppress rebuilding overlay", func(t *testing.T) {
		decision := eventpipeline.DeriveEventExecutionPlanBehavioralDecisionFromEventsWithHooks(
			[]eventpipeline.EventWithHooks{
				{
					Classified:      eventpipeline.ClassifiedEvent{FileType: eventpipeline.FileTypeCriticalCSS},
					RunOnChangeOnly: false,
				},
				{
					Classified:      eventpipeline.ClassifiedEvent{FileType: eventpipeline.FileTypeNormalCSS},
					RunOnChangeOnly: false,
				},
			},
		)
...
func TestBuildWatcherEventLogPayloadsForEventsWithHooks(t *testing.T) {
	...
	t.Run("builds payloads in events-with-hooks order", func(t *testing.T) {
		...
		watcherEventLogPayloads := eventpipeline.BuildWatcherEventLogPayloadsForEventsWithHooks(eventsWithHooks)
...
func TestDeriveWatcherEventFlowDecisionFromPlanningResult(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			decision := eventpipeline.DeriveWatcherEventFlowDecisionFromPlanningResult(testCase.PlanningResult)
	...
	t.Run("config change suppresses execution plan", func(t *testing.T) {
		executionInput := eventpipeline.BuildWatcherEventExecutionInputFromPlanningResult(
			eventpipeline.EventExecutionPlanningResult{
				...
				EventsWithHooks: []eventpipeline.EventWithHooks{
					{
						Classified: eventpipeline.ClassifiedEvent{FileType: eventpipeline.FileTypeOther},
					},
				},
			},
		)
...
func TestPlanBrowserReloadForAction(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			BrowserDecision:  eventpipeline.BrowserPhaseDecision{WaitForApp: true, WaitForVite: true, CycleVite: true},
...
func TestPlanInvalidateViteFallbackBrowserDecision(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			decision := eventpipeline.PlanInvalidateViteFallbackBrowserDecision(testCase.UsingVite)
			if !reflect.DeepEqual(decision, testCase.ExpectedDecision) {
...
func TestDeriveBrowserPhaseExecutionCategory(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			category := eventpipeline.DeriveBrowserPhaseExecutionCategory(testCase.Action)
