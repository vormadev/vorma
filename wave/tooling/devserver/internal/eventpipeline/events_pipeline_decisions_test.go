...
func TestFilterClassifiedEventsForProcessingByPostClassificationDecision(
	...
	if len(filteredClassifiedEvents) != 2 {
		t.Fatalf("expected two included classified events, got %#v", filteredClassifiedEvents)
	}
	if filteredClassifiedEvents[0].Event.Name != "first.txt" {
		t.Fatalf("unexpected first filtered event: %#v", filteredClassifiedEvents[0])
	}
	if filteredClassifiedEvents[1].Event.Name != "second.txt" {
		t.Fatalf("unexpected second filtered event: %#v", filteredClassifiedEvents[1])
	}
}
...
func TestShouldStartAppAfterImplicitBuild(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			ShouldRunImplicitBuild: true,
			Restart:                eventpipeline.RestartPhaseDecision{RestartApp: false},
			ExpectedStartApp:       false,
		},
	}
...
func TestApplyHookStageActionsToWorkSet(t *testing.T) {
	t.Run("nil workset keeps stage actions and zero refresh result", func(t *testing.T) {
		...
		if refreshActionResult.RestartRequested || refreshActionResult.RecompileGo {
			t.Fatalf(
