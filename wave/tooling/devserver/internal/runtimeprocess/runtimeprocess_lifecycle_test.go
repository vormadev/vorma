...
func TestAppProcessManager_StopCurrentApp_DoesNotReturnDoubleWaitError(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("shell script process lifecycle test is unix-specific")
	}
	...
	for iteration := 0; iteration < 20; iteration++ {
		...
		if startError != nil {
			t.Fatalf("StartApp iteration %d returned error: %v", iteration, startError)
		}

		stopError := manager.StopCurrentApp()
		if stopError != nil && strings.Contains(stopError.Error(), "Wait was already called") {
			t.Fatalf(
		...
		if stopError != nil {
			t.Fatalf("StopCurrentApp iteration %d returned error: %v", iteration, stopError)
		}
	}
}
...
func writeLongRunningTestScript(t *testing.T) string {
	...
	scriptPath := filepath.Join(t.TempDir(), "runtimeprocess_lifecycle.sh")
