...
func TestWaitForApp_UsesConfiguredHealthcheckEndpoint(t *testing.T) {
	...
	listener, listenError := net.Listen("tcp", fmt.Sprintf("127.0.0.1:%d", port))
	if listenError != nil {
		t.Skipf(
	...
	appReadyURL := runtimeprocess.ResolveAppReadyURL(
		port,
		cfg.HealthcheckEndpoint(),
	)
	if !runtimeprocess.WaitForAnyReady(
		[]string{appReadyURL},
		runtimeprocess.DefaultReadinessWaitPolicy(),
	) {
		t.Fatal(
			"expected runtimeprocess.WaitForAnyReady to return true when healthcheck endpoint is ready",
		)
	}
}

func TestResolveAppReadyURL_UsesIPv4LoopbackHost(t *testing.T) {
	resolvedURL := runtimeprocess.ResolveAppReadyURL(4242, "/healthz")
