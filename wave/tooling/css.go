...
func (p *cssProcessor) build(nature string, isDev bool) error {
	...
	if nature == "critical" {
		...
		// Clean up old files
		oldFiles, err := filepath.Glob(filepath.Join(outputPath, wave.NormalCSSGlobPattern))
		if err != nil {
		...
		outputFileName = hashBytes(result.OutputFiles[0].Contents, wave.NormalCSSBaseName)
