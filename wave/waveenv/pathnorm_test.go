...
func testWaveOutPath(relativePath string) string {
	return filepath.ToSlash(
		filepath.Join(waveartifacts.HashedOutputDirname, relativePath),
	)
}

func testWaveOutWindowsPath(relativePath string) string {
	return `\` + strings.ReplaceAll(testWaveOutPath(relativePath), "/", `\`)
}
...
func TestResolveFromReferencedPath_NormalizesWindowsSeparators(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			referencedPath:   testWaveOutWindowsPath("styles.hash.css"),
			expectedURL:      "/assets/" + testWaveOutPath("styles.hash.css"),
		},
		{
