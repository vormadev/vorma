...
type Watcher struct {
	...
	ignoredDirs    []string
	ignoredFiles   []string
	defaultWatched []config.WatchedFile
...
func (w *Watcher) setupPatterns() {
	...
	if w.cfg.UsingBrowser() {
		...
		w.defaultWatched = []config.WatchedFile{
			{
...
// Normalizes the path before matching.
func (w *Watcher) FindWatchedFile(path string) *config.WatchedFile {
	np := w.norm(path)
...
func (w *Watcher) HasStrategyHook(wf *config.WatchedFile) bool {
	if wf == nil {
		return false
	}
...
func (w *Watcher) GetFirstStrategy(wf *config.WatchedFile) *config.OnChangeStrategy {
	if wf == nil {
		return nil
	}
	for _, hook := range wf.OnChangeHooks {
