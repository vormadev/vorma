// Package devserver provides the Wave development server with file watching,
...
func (s *server) run() error {
	...
	for {
		...
		s.builder = builder.New(s.cfg, s.log)
...
func (s *server) reloadConfig() error {
	...
	newCfg, err := config.ParseFile(configPath)
	if err != nil {
		return err
	}
...
func (s *server) startRefreshServer(port int) {
	...
	mux.HandleFunc("/get-refresh-script-inner", func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte(runtime.RefreshScriptInner(config.GetRefreshServerPort())))
	})
...
//
// Priority rules:
//  1. Config restarts have absolute priority and cannot be downgraded
//  2. Go-recompile restarts take priority over non-Go restarts
//  3. A weaker request is dropped if a stronger one is pending
//
// This ensures config changes are never lost, even if file changes occur
// between the config change detection and the restart processing.
func (s *server) triggerRestartWithOpts(recompileGo bool, isConfigRestart bool) {
	...
	// We must not replace a config restart with a non-config restart.
	select {
...
func (s *server) waitForApp() bool {
	url := fmt.Sprintf("http://localhost:%d%s", config.MustGetAppPort(), s.cfg.HealthcheckEndpoint())
