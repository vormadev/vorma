...
func (b *Builder) processStaticFiles(opts staticOpts) error {
	if _, err := os.Stat(opts.srcDir); os.IsNotExist(err) {
		...
		if opts.isPublic {
			return b.savePublicFileMapJS(config.FileMap{})
		}
		return nil
	}
	...
	finalMap := make(config.FileMap)
	newMap.Range(func(k, v any) bool {
		finalMap[k.(string)] = v.(config.FileVal)
		return true
	})
	...
	if opts.granular && oldMap != nil {
		oldMap.Range(func(k, v any) bool {
			key := k.(string)
			oldVal := v.(config.FileVal)
			if newVal, exists := newMap.Load(key); !exists || newVal.(config.FileVal).DistName != oldVal.DistName {
...
func (b *Builder) processFile(fi fileInfo, opts staticOpts, newMap, oldMap *sync.Map) error {
	...
	if oldMap != nil {
		if oldVal, ok := oldMap.Load(fi.relPath); ok {
			if oldVal.(config.FileVal).ContentHash == contentHash {
				return nil
			}
		}
	}
...
func (b *Builder) savePublicFileMapJS(fm config.FileMap) error {
