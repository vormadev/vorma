...
func TestDevLock_AcquireBlocksSecondAcquireUntilRelease(t *testing.T) {
	...
	first := wavelock.NewDevLock(staticDir)
	if err := first.Acquire(); err != nil {
		t.Fatalf("first acquire returned error: %v", err)
	}
	defer first.Release()

	second := wavelock.NewDevLock(staticDir)
	err := second.Acquire()
	if err == nil {
		t.Fatal("expected second acquire to fail while first lock is held")
	}
	if !errors.Is(err, wavelock.ErrLockHeld) {
...
func TestIsLockFile(t *testing.T) {
	if !wavelock.IsLockFile(".wave-dev.lock") {
		t.Fatal("expected .wave-dev.lock to be recognized as lock file")
	}
	if !wavelock.IsLockFile(".wave-build.lock") {
		t.Fatal("expected .wave-build.lock to be recognized as lock file")
	}
	if wavelock.IsLockFile(".wave-custom") {
		t.Fatal(
...
func TestBuildLock_AcquireBlocksSecondAcquireUntilRelease(t *testing.T) {
	staticDir := t.TempDir()

	first := wavelock.NewBuildLock(staticDir)
	if err := first.Acquire(); err != nil {
		t.Fatalf("first acquire returned error: %v", err)
	}
	defer first.Release()

	second := wavelock.NewBuildLock(staticDir)
	err := second.Acquire()
	if err == nil {
		t.Fatal("expected second acquire to fail while first lock is held")
	}
...
func TestDevLock_AcquireRecoversStaleLockFile(t *testing.T) {
	staticDir := t.TempDir()
	lock := wavelock.NewDevLock(staticDir)
