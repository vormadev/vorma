...
func (watcher *Watcher) reconcile_watch_dirs(
	...
	if err := filepath.WalkDir(
		...
		func(path string, entry fs.DirEntry, err error) error {
			...
			if entry.IsDir() {
				if slices.Contains(
					[]string{".git", "node_modules", constants.DIST_DIRNAME},
					entry.Name(),
				) {
	...
	for path := range should_watch.Range() {
		...
		if err := _watcher.Add(path.Str()); err != nil {
			return err
		}
	}
...
// Treats CHMOD events for empty files as writes, because it
// has been observed that some editor save workflows
// (truncate-then-write, atomic rename, etc.) can cause
