...
// RefreshScript returns the dev mode browser refresh script wrapped in script tags
func (r *Runtime) RefreshScript() template.HTML {
	if !config.GetIsDev() {
		return ""
	}

	port := config.GetRefreshServerPort()
	if port == 0 {
...
// RefreshScriptHash returns the CSP hash for the refresh script
func (r *Runtime) RefreshScriptHash() string {
	if !config.GetIsDev() {
		return ""
	}
