...
func TestReloadConfig_PreservesFrameworkInjectedFields(t *testing.T) {
	...
	waveframework.StateForConfig(cfg).ProdBuildHook = "go run ./backend/cmd/build --hook"
	waveframework.StateForConfig(cfg).SchemaExtensions = map[string]jsonschema.Entry{
		"Vorma": {
			Type: jsonschema.TypeObject,
		},
	}
	waveframework.StateForConfig(cfg).RunBuildHook = func(context.Context, bool) error { return nil }
	...
	if len(waveframework.StateForConfig(s.Cfg).WatchPatterns) != 1 ||
		waveframework.StateForConfig(s.Cfg).WatchPatterns[0].Pattern != "**/*.route" {
		t.Fatalf(
			"framework watch patterns were not preserved: %#v",
			waveframework.StateForConfig(s.Cfg).WatchPatterns,
		)
	}
	if len(waveframework.StateForConfig(s.Cfg).IgnoredPatterns) != 1 ||
		waveframework.StateForConfig(s.Cfg).IgnoredPatterns[0] != "generated/**" {
		t.Fatalf(
			"framework ignored patterns were not preserved: %#v",
			waveframework.StateForConfig(s.Cfg).IgnoredPatterns,
		)
	}
	if waveframework.StateForConfig(s.Cfg).DevBuildHook != "go run ./backend/cmd/build --dev --hook" {
		t.Fatalf(
			"framework dev build hook was not preserved: %q",
			waveframework.StateForConfig(s.Cfg).DevBuildHook,
		)
	}
	if waveframework.StateForConfig(s.Cfg).ProdBuildHook != "go run ./backend/cmd/build --hook" {
		t.Fatalf(
			"framework prod build hook was not preserved: %q",
			waveframework.StateForConfig(s.Cfg).ProdBuildHook,
		)
	}
	if got := waveframework.StateForConfig(s.Cfg).SchemaExtensions["Vorma"].Type; got != jsonschema.TypeObject {
		t.Fatalf(
			"framework schema extension was not preserved: %#v",
			waveframework.StateForConfig(s.Cfg).SchemaExtensions["Vorma"],
		)
	}
	if waveframework.StateForConfig(s.Cfg).RunBuildHook == nil {
		t.Fatal("framework run build hook was not preserved")
	}
	if err := waveframework.StateForConfig(s.Cfg).RunBuildHook(context.Background(), true); err != nil {
		t.Fatalf("framework run build hook returned error: %v", err)
	}
	if waveframework.StateForConfig(s.Cfg).PrepareGoBuildOverlay == nil {
		t.Fatal("framework go build overlay preparer was not preserved")
	}
	overlay, overlayErr := waveframework.StateForConfig(s.Cfg).PrepareGoBuildOverlay()
	if overlayErr != nil {
		t.Fatalf(
...
func TestReloadConfig_UsesConfigLocationWhenAvailable(t *testing.T) {
			...
			"Core": map[string]any{
				...
				"StaticAssetDirs": map[string]any{
					"Public": pathRelativeToCurrentWorkingDirectoryForToolingConfigJSON(
						t,
						filepath.Join(root, waveartifacts.PublicDirname),
					),
					"Private": pathRelativeToCurrentWorkingDirectoryForToolingConfigJSON(
						t,
						filepath.Join(root, waveartifacts.PrivateDirname),
					),
				},
			},
		},
	...
	if marshalError != nil {
		t.Fatalf("failed marshaling config JSON: %v", marshalError)
	}
	if err := os.MkdirAll(filepath.Dir(configPath), 0755); err != nil {
		t.Fatalf("failed creating config parent directory: %v", err)
	}
...
func TestReloadConfigIfChanged_FirstNoOpWriteWithRelativeConfigPathIsNoOp(
	...
	currentConfig, parseError := waveconfig.ParseConfigFile(configAbsolutePath)
	if parseError != nil {
