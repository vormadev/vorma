...
func startBlockingViteInvalidateServerForBroadcastBehaviorTests(
	...
	invalidateServer := &http.Server{
		Handler: http.HandlerFunc(
			func(responseWriter http.ResponseWriter, request *http.Request) {
				if request.URL.Path != waveframework.ViteFileMapChangedNotifyEndpointPath {
					responseWriter.WriteHeader(http.StatusNotFound)
					return
				}
...
func TestBroadcastReload_WaitAppDoesNotBlockSubsequentReloads(
	...
	t.Setenv(waveenv.EnvPort, strconv.Itoa(healthServerPort))
...
func TestBroadcastReload_FrameworkRuntimeReloadRequestsAreAsyncAndCleanupCancelable(
	...
	t.Setenv(waveenv.EnvMode, waveenv.EnvModeDev)
	t.Setenv(waveenv.EnvPort, strconv.Itoa(reloadServerPort))
	t.Setenv(waveenv.EnvPortSet, "true")
...
func TestBroadcastReload_FrameworkRuntimeReloadSuccessBroadcastsPayloadAndHeaders(
	...
	case capturedHeaders := <-requestHeaders:
		if got, want := capturedHeaders.Get(waveframework.FrameworkRuntimeReloadAttemptIDHeaderName), "attempt-42"; got != want {
			t.Fatalf("reload attempt header=%q, want %q", got, want)
		}
		if got, want := capturedHeaders.Get(waveframework.FrameworkRuntimeReloadExpectedBuildIDHeaderName), "build-42"; got != want {
			t.Fatalf("expected build ID header=%q, want %q", got, want)
		}
		if got, want := capturedHeaders.Get(waveframework.FrameworkRuntimeReloadTriggerHeaderName), "template-watch"; got != want {
