...
func TestNotifyViteFileMapChanged_ReturnsErrorWhenViteNotRunning(
	t *testing.T,
) {
	s := &Server{Log: newDiscardLogger()}
	err := s.NotifyViteFileMapChanged()
	if err == nil {
...
func TestNotifyViteFileMapChanged_ReturnsSuccessOn200(t *testing.T) {
	...
	if err := s.NotifyViteFileMapChanged(); err != nil {
...
func TestNotifyViteFileMapChanged_ReturnsErrorOnNon200(t *testing.T) {
	testServer := httptest.NewServer(
		http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestNotifyViteFileMapChanged_Primary404ReturnsErrorWithoutRetry(
	t *testing.T,
) {
	...
	callError := s.NotifyViteFileMapChanged()
	if callError == nil {
	...
	if requestedPaths[0] != waveframework.ViteFileMapChangedNotifyEndpointPath {
		t.Fatalf(
...
func TestNotifyViteFileMapChanged_Primary500DoesNotCallFallback(t *testing.T) {
...
func TestCallFrameworkRuntimeReloadEndpointWithContext_RequiresEndpointPath(
	...
	s := &Server{
		Log:          newDiscardLogger(),
		PortResolver: waveenv.NewResolver(),
	}
	t.Setenv(waveenv.EnvMode, waveenv.EnvModeDev)
	t.Setenv(waveenv.EnvPort, "8080")
...
func TestCallFrameworkRuntimeReloadEndpointWithContext_SetsHeadersAndReturnsSuccess(
	...
	t.Setenv(waveenv.EnvMode, waveenv.EnvModeDev)
	t.Setenv(waveenv.EnvPort, strconv.Itoa(port))
	t.Setenv(waveenv.EnvPortSet, "true")
	...
	if got, want := requestHeaders.Get(waveframework.FrameworkRuntimeReloadAttemptIDHeaderName), "attempt-77"; got != want {
		t.Fatalf("reload attempt header=%q, want %q", got, want)
	}
	if got, want := requestHeaders.Get(waveframework.FrameworkRuntimeReloadExpectedBuildIDHeaderName), "build-77"; got != want {
		t.Fatalf("expected build ID header=%q, want %q", got, want)
	}
	if got, want := requestHeaders.Get(waveframework.FrameworkRuntimeReloadTriggerHeaderName), "routes-watch"; got != want {
...
func TestServerPortUsesOwnedResolverState(t *testing.T) {
	...
	s.PortResolver = waveenv.NewResolver()
