...
func TestBuild_BrowserModeCanSkipPostHookPublicStaticProcessing(
	t *testing.T,
) {
	root := t.TempDir()
	config := newParsedConfigForBuilderBasicTestsAtRoot(root)
	...
	if buildError != nil {
		t.Fatalf(
			"expected build to skip public-static processing and succeed, got: %v",
			buildError,
		)
	}
	...
	if _, statError := os.Stat(config.Dist.PublicFileMapGob()); !os.IsNotExist(
		...
		t.Fatalf(
			"expected no post-hook public filemap gob output when public-static processing is skipped, stat error: %v",
			statError,
		)
	}
}
...
func TestProcessPublicFilesOnly_WritesCanonicalPublicFileMapJSONAndRef(
	t *testing.T,
) {
	root := t.TempDir()
	config := newParsedConfigForBuilderBasicTestsAtRoot(root)
	...
	if readJSONError != nil {
		t.Fatalf(
			"read canonical public filemap JSON %q: %v",
			canonicalJSONPath,
			readJSONError,
		)
	}
