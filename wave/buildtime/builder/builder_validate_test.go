...
func TestValidateWatchedFile_RunOnChangeOnlyTimingRules(t *testing.T) {
	t.Run(
		"allows non-run-on-change-only regardless of timing",
		func(t *testing.T) {
			wf := &wavewatch.WatchedFile{
				Pattern:         "**/*.go",
				RunOnChangeOnly: false,
				OnChangeHooks: []wavewatch.OnChangeHook{
					{Cmd: "echo hello", Timing: wavewatch.OnChangeStrategyPost},
				},
			}
	...
	t.Run(
		"allows pre timing and default timing when run-on-change-only",
		func(t *testing.T) {
			wf := &wavewatch.WatchedFile{
				Pattern:         "**/*.go",
				...
				OnChangeHooks: []wavewatch.OnChangeHook{
					{Cmd: "echo hello"},
					{Cmd: "echo world", Timing: wavewatch.OnChangeStrategyPre},
				},
			}
			...
			OnChangeHooks: []wavewatch.OnChangeHook{
				{
					...
					Callback: func(*wavewatch.HookContext) (*wavewatch.RefreshAction, error) { return nil, nil },
				},
			},
		}
	...
	t.Run(
		"allows run-combined-dev-build-hook commands with pre timing when run-on-change-only",
		func(t *testing.T) {
			wf := &wavewatch.WatchedFile{
				Pattern:         "**/*.go",
				RunOnChangeOnly: true,
	...
	t.Run(
		"rejects non-pre command timing when run-on-change-only",
		func(t *testing.T) {
			wf := &wavewatch.WatchedFile{
				Pattern:         "**/*.go",
				RunOnChangeOnly: true,
	...
	t.Run(
		"rejects cmd and run-combined-dev-build-hook combination",
		func(t *testing.T) {
			wf := &wavewatch.WatchedFile{
				Pattern:         "**/*.go",
				RunOnChangeOnly: false,
	...
	t.Run(
		"rejects simultaneous disable-stage-timeout and per-hook-timeout",
		func(t *testing.T) {
			wf := &wavewatch.WatchedFile{
				Pattern: "**/*.go",
				OnChangeHooks: []wavewatch.OnChangeHook{
	...
	t.Run(
		"rejects simultaneous disable-stage-callback-timeout and per-hook-callback-timeout",
		func(t *testing.T) {
			wf := &wavewatch.WatchedFile{
				Pattern: "**/*.go",
				OnChangeHooks: []wavewatch.OnChangeHook{
...
func TestValidateConfig_HookCommandTimeoutValidation(t *testing.T) {
	...
	t.Run("rejects negative concurrent timeout", func(t *testing.T) {
		...
		if !strings.Contains(
			err.Error(),
			"ConcurrentCommandTimeoutMilliseconds",
		) {
			t.Fatalf("unexpected error message: %v", err)
		}
	})
	...
	t.Run("rejects negative concurrent-no-wait timeout", func(t *testing.T) {
		...
		if !strings.Contains(
			err.Error(),
			"ConcurrentNoWaitCommandTimeoutMilliseconds",
		) {
			t.Fatalf("unexpected error message: %v", err)
		}
	})
...
func TestValidateConfig_RejectsInvalidWatchGlobPatterns(t *testing.T) {
	...
	t.Run("rejects invalid OnChangeHooks exclude pattern", func(t *testing.T) {
		...
		if !strings.Contains(
			err.Error(),
			"Watch.Include[0].OnChangeHooks[0].Exclude[0]",
		) {
			t.Fatalf("unexpected error message: %v", err)
		}
	})
...
func TestValidateConfig_HookCallbackTimeoutValidation(t *testing.T) {
	...
	t.Run("rejects negative concurrent callback timeout", func(t *testing.T) {
		...
		if !strings.Contains(
			err.Error(),
			"ConcurrentCallbackTimeoutMilliseconds",
		) {
			t.Fatalf("unexpected error message: %v", err)
		}
	})
	...
	t.Run(
		...
		func(t *testing.T) {
			...
			if !strings.Contains(
				err.Error(),
				"ConcurrentNoWaitCallbackTimeoutMilliseconds",
			) {
				t.Fatalf("unexpected error message: %v", err)
			}
		},
