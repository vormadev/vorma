...
func TestDeriveWatcherEventPreClassificationDecision(t *testing.T) {
	...
	if emptyPathDecision.IncludeEvent {
		t.Fatalf(
			"expected empty-path event to be excluded, got %#v",
			emptyPathDecision,
		)
	}
	...
	lockFileDecision := classification.DerivePreClassificationDecision(
		fsnotify.Event{
			Name: filepath.Join("tmp", ".wave.lock"),
			Op:   fsnotify.Write,
		},
		dependencies,
	)
	...
	tempFileDecision := classification.DerivePreClassificationDecision(
		fsnotify.Event{
			Name: filepath.Join("tmp", "editor.tmp"),
			Op:   fsnotify.Write,
		},
		dependencies,
	)
	...
	ignoredPathDecision := classification.DerivePreClassificationDecision(
		fsnotify.Event{
			Name: filepath.Join("tmp", "ignored.txt"),
			Op:   fsnotify.Write,
		},
		dependencies,
	)
	...
	if !regularFileDecision.IncludeEvent {
		t.Fatalf(
			"expected regular file write to be included, got %#v",
			regularFileDecision,
		)
	}
	...
	if makeDirectoryError := os.MkdirAll(directoryPath, 0o755); makeDirectoryError != nil {
		t.Fatalf(
			"failed creating directory for classification test: %v",
			makeDirectoryError,
		)
	}
	...
	if makeDirectoryError := os.MkdirAll(emptyCreatedDirectoryPath, 0o755); makeDirectoryError != nil {
		t.Fatalf(
			"failed creating empty directory for create classification test: %v",
			makeDirectoryError,
		)
	}
	...
	if makeDirectoryError := os.MkdirAll(
		...
		t.Fatalf(
			"failed creating non-empty directory for create classification test: %v",
			makeDirectoryError,
		)
	}
	if writeError := os.WriteFile(
		...
		t.Fatalf(
			"failed writing non-empty directory fixture file: %v",
			writeError,
		)
	}
...
func TestBuildWatcherEventPreClassificationPlanFromEvents(t *testing.T) {
	...
	if !reflect.DeepEqual(includedEvents, expectedIncludedEvents) {
		t.Fatalf(
			"included events=%#v, want %#v",
			includedEvents,
			expectedIncludedEvents,
		)
	}
}
...
func TestWatcherEventClassificationProber_CachesConfigProbeByPath(
	t *testing.T,
) {
	root := t.TempDir()
	...
	if !classification.IsConfigurationPathChange(
...
func TestWatcherEventClassificationProber_CachesDirectoryProbeByPath(
	t *testing.T,
) {
	root := t.TempDir()
	directoryPath := filepath.Join(root, waveartifacts.AssetsDirname)
...
func TestWatcherEventClassificationProber_SharesSingleSnapshotAcrossProbeTypes(
	...
	canonicalFromSymlinkPath := classification.CanonicalizePathForEvent(
		symlinkPath,
	)
...
func TestDeriveWatcherEventPostClassificationDecision(t *testing.T) {
	ignoredDecision := classification.DerivePostClassificationDecision(
		true,
		false,
	)
	if ignoredDecision.IncludeClassifiedEvent ||
		!ignoredDecision.DroppedBecauseIgnored {
		t.Fatalf(
	...
	chmodDecision := classification.DerivePostClassificationDecision(
		false,
		true,
	)
	if chmodDecision.IncludeClassifiedEvent ||
		!chmodDecision.DroppedBecauseChmod {
		t.Fatalf(
	...
	includedDecision := classification.DerivePostClassificationDecision(
		false,
		false,
	)
