...
func DeduplicateWatcherEvents(
	...
	for eventIndex := range rawEvents {
		...
		normalizedPath := NormalizeWatcherEventPathForDeduplication(
			rawEvent.Name,
		)
		if normalizedPath == "" {
			continue
...
func MergeWatchedFiles(matches []*wavewatch.WatchedFile) *wavewatch.WatchedFile {
	if len(matches) == 0 {
		return nil
	}

	merged := &wavewatch.WatchedFile{
	...
	allHooks := make([]wavewatch.OnChangeHook, 0)
...
func NormalizeWatcherEventPathForDeduplication(
	...
	cleanedEventPath := waveenv.TrimAndCleanPath(eventPath)
	if cleanedEventPath == "" {
		return ""
	}
...
func canonicalizeAbsolutePathForWatcherEventDeduplication(
	...
	return waveenv.CanonicalizePathForLocationComparison(absolutePath)
}
...
func MissingFileAliasKeyForWatcherEventDeduplication(
	...
	parentDirectoryPath := waveenv.AbsoluteDirectory(absolutePath)
