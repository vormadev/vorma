...
func TestDeduplicateWatcherEvents_PrefersRemoveOverWrite(t *testing.T) {
	...
	result := dedup.DeduplicateWatcherEvents(
		rawEvents,
	...
	if !foundRemove {
		t.Fatalf(
			"expected /tmp/a.txt event to retain remove op, got %#v",
			result.Events,
		)
	}
}
...
func TestDeduplicateWatcherEventsByPath_MergesOpsAndSorts(t *testing.T) {
	...
	if len(deduplicatedEvents) != 2 {
		t.Fatalf(
			"expected 2 deduplicated events, got %d",
			len(deduplicatedEvents),
		)
	}
	if deduplicatedEvents[0].Name != "/tmp/a.txt" ||
		deduplicatedEvents[1].Name != "/tmp/b.txt" {
	...
	if !deduplicatedEvents[0].Has(fsnotify.Create) ||
		...
		t.Fatalf(
			"expected merged ops for /tmp/a.txt, got %#v",
...
func TestFilterAndGroupWatcherEvents(t *testing.T) {
	...
	if _, exists := groupedEvents["/tmp/project"]; !exists {
		t.Fatalf(
			"expected grouped events to include /tmp/project, got %#v",
			groupedEvents,
		)
	}
	if _, exists := groupedEvents["/tmp/project/nested"]; !exists {
		t.Fatalf(
			"expected grouped events to include /tmp/project/nested, got %#v",
			groupedEvents,
		)
	}
}
...
func TestBuildCanonicalPathAliasSetAndContains(t *testing.T) {
	...
	if !dedup.PathAliasContains(aliasSet, strings.ToLower(absolutePath)) {
		t.Fatalf(
			"expected alias set to contain lowercase path %q",
...
func TestWatcherEventDeduplicationIndex_RecordAndResolve(t *testing.T) {
	...
	resolvedPathKey := index.ResolveExistingPathKey(
