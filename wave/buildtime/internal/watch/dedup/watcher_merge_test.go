...
func TestMergeWatchedFiles_UnionSemantics(t *testing.T) {
	wfFramework := &wavewatch.WatchedFile{
		Pattern:                            "**/*.go",
	...
	merged := dedup.MergeWatchedFiles([]*wavewatch.WatchedFile{wfFramework, wfUser})
	if merged == nil {
	...
	if len(merged.SortedHooks.Pre) != 1 ||
		merged.SortedHooks.Pre[0].Cmd != "framework-pre" {
		t.Fatalf("unexpected sorted pre Hooks: %#v", merged.SortedHooks.Pre)
	}
	if len(merged.SortedHooks.Post) != 1 ||
		merged.SortedHooks.Post[0].Cmd != "user-post" {
		t.Fatalf("unexpected sorted post Hooks: %#v", merged.SortedHooks.Post)
	}
}

func TestMergeWatchedFiles_EmptyInput(t *testing.T) {
	if got := dedup.MergeWatchedFiles(nil); got != nil {
		t.Fatalf("dedup.MergeWatchedFiles(nil) = %#v, want nil", got)
	}
}
