...
func TestNewWatcher_FrameworkIgnoredPatternsAreRelativeToResolveRoot(
	...
	if !watcher.IsIgnoredDir(ignoredDir) {
		t.Fatalf(
			"expected %q to be ignored as framework dir pattern",
			ignoredDir,
		)
	}
	...
	if !watcher.IsIgnoredFile(ignoredFile) {
		t.Fatalf(
			"expected %q to be ignored as framework file pattern",
			ignoredFile,
		)
	}
}

func TestNewWatcher_FrameworkIgnoredLiteralDirectoryPatternIgnoresDirectoryTree(
	t *testing.T,
) {
	root := t.TempDir()
	cfg := newParsedConfigForWatchTestsAtRoot(t, root)
	...
	if !watcher.IsIgnoredDir(ignoredDirectoryPath) {
		t.Fatalf(
			"expected %q to be ignored as framework literal directory pattern",
	...
	if !watcher.IsIgnoredFile(ignoredFilePath) {
		t.Fatalf(
			"expected file %q to be ignored as framework literal directory pattern",
			ignoredFilePath,
		)
	}
}
...
func TestFindWatchedFile_MergesFrameworkAndUserMatches(t *testing.T) {
	...
	waveframework.StateForConfig(cfg).WatchPatterns = []wavewatch.WatchedFile{
		{
			...
			OnChangeHooks: []wavewatch.OnChangeHook{
				{Cmd: "framework-pre", Timing: wavewatch.OnChangeStrategyPre},
			},
		},
	}
	...
	if len(watchedFile.OnChangeHooks) != 2 {
		t.Fatalf(
			"expected 2 merged hooks, got %d",
			len(watchedFile.OnChangeHooks),
		)
	}
	if watchedFile.OnChangeHooks[0].Cmd != "framework-pre" {
		t.Fatalf(
			"expected first hook to be framework hook, got %q",
			watchedFile.OnChangeHooks[0].Cmd,
		)
	}
	if watchedFile.OnChangeHooks[1].Cmd != "user-post" {
		t.Fatalf(
			"expected second hook to be user hook, got %q",
			watchedFile.OnChangeHooks[1].Cmd,
		)
	}
}

func TestNewWatcher_DoesNotMutateConfigWatchPatternsOrHookExcludes(
	t *testing.T,
) {
	root := t.TempDir()
	cfg := newParsedConfigForWatchTestsAtRoot(t, root)
	...
	if waveframework.StateForConfig(cfg).WatchPatterns[0].Pattern != "framework/**/*.txt" {
		t.Fatalf(
			"expected framework pattern to remain relative, got %q",
			waveframework.StateForConfig(cfg).WatchPatterns[0].Pattern,
		)
	}
	if waveframework.StateForConfig(cfg).WatchPatterns[0].OnChangeHooks[0].Exclude[0] != "framework/exclude/**" {
		t.Fatalf(
	...
	if frameworkMatch == nil {
		t.Fatal(
			"expected framework pattern to match after watcher normalization",
		)
	}

	userMatch := watcher.FindWatchedFile(
		filepath.Join(root, "user", "file.txt"),
	)
	if userMatch == nil {
...
func TestWatcherStaticClassificationUsesDirectoryBoundaries(t *testing.T) {
	...
	publicPrefixCollision := filepath.Join(
		root,
		"static",
		"publicity",
		"app.js",
	...
	if watcher.IsPublicStaticFile(publicPrefixCollision) {
		t.Fatalf(
			"did not expect %q to be classified as public static file",
			publicPrefixCollision,
		)
	}
...
func TestWatcherStaticClassificationRecognizesSymlinkAliasPaths(
	...
	if makeDirectoryError := os.MkdirAll(privateStaticDirectoryPath, 0o755); makeDirectoryError != nil {
		t.Fatalf(
			"failed creating private static directory: %v",
			makeDirectoryError,
		)
	}
	...
	if symlinkError := os.Symlink(
		...
		t.Skipf(
			"symlink creation is unavailable on this platform: %v",
			symlinkError,
		)
	}
...
func TestNewWatcher_RejectsInvalidFrameworkWatchPattern(t *testing.T) {
	...
	if err == nil {
		t.Fatal(
			"expected newWatcher to fail for invalid framework watch pattern",
		)
	}
...
func TestNewWatcher_RejectsEmptyFrameworkWatchPattern(t *testing.T) {
	...
	if !strings.Contains(
		err.Error(),
		"FrameworkWatchPatterns[0].Pattern is required",
	) {
		t.Fatalf("unexpected error: %v", err)
	}
}
...
func TestNewWatcher_RejectsInvalidFrameworkHookExcludePattern(t *testing.T) {
	...
	if err == nil {
		t.Fatal(
			"expected newWatcher to fail for invalid framework hook exclude pattern",
		)
	}
	if !strings.Contains(
		err.Error(),
		"FrameworkWatchPatterns[0].OnChangeHooks[0].Exclude[0]",
	) {
		t.Fatalf("unexpected error: %v", err)
	}
}
...
func TestNewWatcher_RejectsInvalidIgnoredPattern(t *testing.T) {
	...
	if err == nil {
		t.Fatal(
			"expected newWatcher to fail for invalid framework ignored pattern",
		)
	}
...
func TestNewWatcher_RejectsIgnoredPatternWithSurroundingWhitespace(
	t *testing.T,
) {
	root := t.TempDir()
	cfg := newParsedConfigForWatchTestsAtRoot(t, root)
	...
	if err == nil {
		t.Fatal(
			"expected newWatcher to fail for ignored pattern with surrounding whitespace",
		)
	}
	if !strings.Contains(
		err.Error(),
		"FrameworkIgnoredPatterns[0] must not include surrounding whitespace",
	) {
		t.Fatalf("unexpected error: %v", err)
	}
}
...
func TestWatcher_RelativeResolveRootMatchesAbsoluteEventPaths(t *testing.T) {
	...
	if makeDirectoryError := os.MkdirAll(
		filepath.Join(root, "static", waveartifacts.PrivateDirname),
		0o755,
	); makeDirectoryError != nil {
		t.Fatalf(
			"failed creating private static directory: %v",
	...
	if getWorkingDirectoryError != nil {
		t.Fatalf(
			"failed to read working directory: %v",
	...
	if absoluteChangedPathError != nil {
		t.Fatalf(
			"failed resolving absolute changed file path: %v",
			absoluteChangedPathError,
		)
	}
	...
	absolutePublicStaticPath, absolutePublicStaticPathError := filepath.Abs(
		filepath.Join("static", waveartifacts.PublicDirname, "app.js"),
	)
	if absolutePublicStaticPathError != nil {
		t.Fatalf(
			"failed resolving absolute public static path: %v",
			absolutePublicStaticPathError,
		)
	}
	...
	if absoluteDistOutputPathError != nil {
		t.Fatalf(
			"failed resolving absolute dist output path: %v",
			absoluteDistOutputPathError,
		)
	}
...
func TestWatcher_AbsoluteGlobPatternsMatchAbsoluteEventPaths(t *testing.T) {
	...
	if absoluteIgnoredPathError != nil {
		t.Fatalf(
			"failed resolving absolute ignored file path: %v",
			absoluteIgnoredPathError,
		)
	}
