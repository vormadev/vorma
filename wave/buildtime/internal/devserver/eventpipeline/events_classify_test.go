...
func TestNeedsHardReload(t *testing.T) {
	...
	if !eventpipeline.NeedsHardReload(
		&wavewatch.WatchedFile{RecompileGoBinary: true},
	) {
...
func TestClassifyEventWithWatcherAndBuilder_CriticalAndNormalCSSFiles(
	...
	criticalCSSFilePath := filepath.Join(
		root,
		"styles",
		waveartifacts.CriticalCSSFileName,
	)
	normalCSSFilePath := filepath.Join(root, "styles", "normal.css")
	sharedCSSFilePath := filepath.Join(root, "styles", "wavebase.css")
	...
	if writeCriticalError := os.WriteFile(
		criticalCSSFilePath,
		[]byte(`@import "./wavebase.css"; .critical { color: red; }`),
		0o644,
	...
	if writeNormalError := os.WriteFile(
		normalCSSFilePath,
		[]byte(`@import "./wavebase.css"; .normal { color: blue; }`),
		0o644,
...
func TestClassifyEventWithWatcherAndBuilder_SiteStyleFileMatrix(t *testing.T) {
	...
	testCases := []struct {
		...
		expectWatched   bool
		assertWatched   func(t *testing.T, watchedFile *wavewatch.WatchedFile)
	}{
		{
		...
		},
		{
			...
			assertWatched: func(t *testing.T, watchedFile *wavewatch.WatchedFile) {
				...
				if watchedFile.OnChangeHooks[0].Timing != wavewatch.OnChangeStrategyPost {
					t.Fatalf(
