...
func TestApplyHookStageActionsToWorkSet(t *testing.T) {
	t.Run(
		...
		func(t *testing.T) {
			stageActions := []wavewatch.RefreshAction{
				{ReloadBrowser: true},
				{WaitForApp: true},
			}
...
func TestDeriveEventsWithHooksForExecution(t *testing.T) {
	t.Run("batch hard reload uses copied hook contexts", func(t *testing.T) {
		firstHookContext := &wavewatch.HookContext{FilePath: "first.txt"}
		secondHookContext := &wavewatch.HookContext{FilePath: "second.txt"}
