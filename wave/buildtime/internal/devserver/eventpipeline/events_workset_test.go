...
func TestWorkSetAddFromRefreshAction(t *testing.T) {
	...
	work.AddFromRefreshAction(wavewatch.RefreshAction{
		TriggerRestart: true,
		RecompileGo:    true,
		ReloadBrowser:  true,
		WaitForApp:     true,
...
func TestDeriveRefreshActionWorkMutationDecision(t *testing.T) {
	testCases := []struct {
		Name                 string
		Action               wavewatch.RefreshAction
...
func TestWorkSetApplyRefreshActions(t *testing.T) {
	t.Run(
		...
		func(t *testing.T) {
			...
			result := work.ApplyRefreshActions([]wavewatch.RefreshAction{
				{ReloadBrowser: true},
				{TriggerRestart: true},
				{WaitForApp: true},
			})
...
func TestDeriveImplicitWorkDecisionForClassifiedEvent(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			ClassifiedEvent: eventpipeline.ClassifiedEvent{
				FileType:    eventpipeline.FileTypeGo,
				WatchedFile: &wavewatch.WatchedFile{RunOnChangeOnly: true},
			},
			ExpectedDecision: eventpipeline.ImplicitWorkDecision{},
		},
		{
		...
		},
		{
			...
			ClassifiedEvent: eventpipeline.ClassifiedEvent{
				...
				WatchedFile: &wavewatch.WatchedFile{RestartApp: true},
			},
			ExpectedDecision: eventpipeline.ImplicitWorkDecision{
		...
		},
		{
			...
			ClassifiedEvent: eventpipeline.ClassifiedEvent{
				...
				WatchedFile: &wavewatch.WatchedFile{RecompileGoBinary: true},
			},
			ExpectedDecision: eventpipeline.ImplicitWorkDecision{
				BuildCriticalCSS: true,
				BuildNormalCSS:   true,
...
func TestWorkSetApplyImplicitWorkDecision_MergesBuildRestartAndPathWork(
	...
	expectedPublicPaths := []string{waveenv.Absolute("/tmp/public/logo.svg")}
	if !reflect.DeepEqual(
	...
	expectedPrivatePaths := []string{
		waveenv.Absolute("/tmp/private/home.html"),
	}
	if !reflect.DeepEqual(
...
func TestWorkSetAddImplicitWork(t *testing.T) {
	...
	t.Run(
		...
		func(t *testing.T) {
			...
			expectedNormalizedPath := waveenv.Absolute(canonicalFilePath)
