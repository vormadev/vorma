...
func TestDeduplicateWatcherEventsByPath_PrefersFirstSeenAliasPathKey(
	...
	testCases := []struct {
	...
	}{
		{
			...
			ExpectedPathKey:  waveenv.Absolute(aliasFilePath),
		...
		},
		{
			...
			ExpectedPathKey:  waveenv.Absolute(targetFilePath),
			ExpectedEventOps: fsnotify.Create | fsnotify.Write,
		},
	}
...
func TestWatcherEventDeduplicationIndex_MemoizesCanonicalAndAliasResolution(
	...
	absoluteAliasFilePath := waveenv.Absolute(aliasFilePath)
	absoluteAliasMissingFilePath := waveenv.Absolute(aliasMissingFilePath)
...
func TestWatcherEventDeduplicationIndex_RecordPathKeyPreservesFirstSeenProbePathKey(
	...
	firstCanonicalAliasPath := waveenv.Absolute(
		filepath.Join(aliasDirectoryPathA, "styles.css"),
	)
	secondCanonicalAliasPath := waveenv.Absolute(
	...
	firstMissingAliasPath := waveenv.Absolute(
		filepath.Join(aliasDirectoryPathA, "missing.css"),
	)
	secondMissingAliasPath := waveenv.Absolute(
...
func TestNormalizeHookContextPathShape_PathShapes(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			RawPath:      "  ./backend/./main.go  ",
			ExpectedPath: waveenv.Absolute("./backend/main.go"),
		},
		{
...
func pathsEquivalentForDedupContract(pathA string, pathB string) bool {
	...
	if filepath.IsAbs(normalizedPathA) &&
		...
		waveenv.PathsReferToSameLocation(normalizedPathA, normalizedPathB) {
		return true
	}
...
func TestBuildEventHooksForProcessingDeduplicatesHooksByPattern(
	...
	if got, want := eventsWithHooks[0].HookCtx.ChangedFilePaths, []string{
		waveenv.Absolute("a.go"),
		waveenv.Absolute("b.txt"),
	...
	cfg.Watch.Include = []wavewatch.WatchedFile{
		{
			...
			OnChangeHooks: []wavewatch.OnChangeHook{
				...
				{
					Timing: wavewatch.OnChangeStrategyPre,
					Callback: func(*wavewatch.HookContext) (*wavewatch.RefreshAction, error) {
