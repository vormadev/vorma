...
type WorkSet struct {
	...
	FrameworkRuntimeReloadRequests []wavewatch.FrameworkRuntimeReloadRequest
...
type RefreshActionWorkMutationDecision struct {
	...
	WaitForApp                    bool
	WaitForVite                   bool
	FrameworkRuntimeReloadRequest *wavewatch.FrameworkRuntimeReloadRequest
}

// RefreshActionReductionDecision captures staged reduction metadata.
type RefreshActionReductionDecision struct {
	ActionsBeforeRestart []wavewatch.RefreshAction
...
func (work *WorkSet) AddFromRefreshAction(
	refreshAction wavewatch.RefreshAction,
...
func NeedsHardReload(watchedFile *wavewatch.WatchedFile) bool {
	if watchedFile == nil {
		return false
	}
...
func appendOrMergeFrameworkRuntimeReloadRequestByEndpoint(
	existingRequests []wavewatch.FrameworkRuntimeReloadRequest,
	request wavewatch.FrameworkRuntimeReloadRequest,
...
func mergeFrameworkRuntimeReloadRequestMetadata(
	existingRequest wavewatch.FrameworkRuntimeReloadRequest,
	incomingRequest wavewatch.FrameworkRuntimeReloadRequest,
) wavewatch.FrameworkRuntimeReloadRequest {
...
func (work *WorkSet) ApplyRefreshActions(
	refreshActions []wavewatch.RefreshAction,
) RefreshActionApplicationResult {
...
func HasPathAlias(path string, aliases []string) bool {
	for _, alias := range aliases {
		if waveenv.PathsReferToSameLocation(path, alias) {
			return true
		}
	}
	return false
