...
func TestBuildNormalizedChangedFilePathsByWatchedPatternForHookContexts(
	...
	root := t.TempDir()
	pattern := filepath.ToSlash(filepath.Join(root, waveartifacts.AssetsDirname, "**", "*.css"))
	firstPath := filepath.Join(root, waveartifacts.AssetsDirname, "first.css")
	secondPath := filepath.Join(root, waveartifacts.AssetsDirname, "nested", "second.css")
...
func TestBuildSkipDuplicateHooksByClassifiedEventIndex(t *testing.T) {
	...
	firstPattern := filepath.ToSlash(
		filepath.Join(root, waveartifacts.AssetsDirname, "**", "*.css"),
	)
	...
	classifiedEvents := []eventpipeline.ClassifiedEvent{
		{
			WatchedFile: &wavewatch.WatchedFile{Pattern: firstPattern},
		},
		{
			WatchedFile: &wavewatch.WatchedFile{Pattern: firstPattern},
		},
		{
			WatchedFile: &wavewatch.WatchedFile{Pattern: secondPattern},
		},
		{
			WatchedFile: nil,
		},
		...
		{
			WatchedFile: &wavewatch.WatchedFile{Pattern: " "},
		},
	}
...
func TestBuildEventHooksForProcessing(t *testing.T) {
	root := t.TempDir()
	pattern := filepath.ToSlash(filepath.Join(root, waveartifacts.AssetsDirname, "**", "*.css"))
	firstEventPath := filepath.Join(root, waveartifacts.AssetsDirname, "first.css")
	secondEventPath := filepath.Join(root, waveartifacts.AssetsDirname, "second.css")
...
func TestHookContextAndErrorUtilities(t *testing.T) {
	...
	originalHookContext := &wavewatch.HookContext{
...
func TestHookCallbackAndContextExecutionUtilities(t *testing.T) {
	...
	okAction, okError := hooks.ExecuteHookCallbackSafely(
		func(_ *wavewatch.HookContext) (*wavewatch.RefreshAction, error) {
...
func TestActionOrderingAndStageActionApplication(t *testing.T) {
	...
	applied := hooks.ApplyHookStageActionsToWorkSet(
		work,
		[]wavewatch.RefreshAction{{ReloadBrowser: true, WaitForApp: true}},
	)
	...
	actionAndErrorResult := hooks.RunAndApplyHookStageActionsAndErrorsToWorkSet(
		hooks.HookStageTypePre,
		func() ([]wavewatch.RefreshAction, []error) {
			return []wavewatch.RefreshAction{
					{ReloadBrowser: true},
				}, []error{
