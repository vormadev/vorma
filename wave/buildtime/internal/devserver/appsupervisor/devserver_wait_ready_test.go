...
func TestWaitForReady_RetriesUntilServerIsHealthy(t *testing.T) {
	...
	policy := appsupervisor.ReadinessWaitPolicy{
	...
	if !appsupervisor.WaitForAnyReady([]string{testServer.URL}, policy) {
...
func TestWaitForAnyReadyWithContext_CancellationStopsWaitEarly(t *testing.T) {
	...
	go func() {
		waitResultCh <- appsupervisor.WaitForAnyReadyWithContext(
			readinessContext,
			[]string{testServer.URL},
			appsupervisor.ReadinessWaitPolicy{
...
func TestWaitForAnyReadyWithContext_MaximumTotalWaitCancelsInFlightProbe(
	...
	waitResult := appsupervisor.WaitForAnyReadyWithContext(
		context.Background(),
		[]string{testServer.URL},
		appsupervisor.ReadinessWaitPolicy{
			HTTPClientTimeout: 5 * time.Second,
	...
	if waitResult {
		t.Fatal(
			"expected readiness wait to return false when total wait budget expires",
		)
	}
