...
func ExecuteInvalidateAsync(options ExecuteInvalidateAsyncOptions) {
	...
	launchAsyncWork(
		func(cycleContext context.Context) {
			...
			if options.NotifyViteFileMapChangedWithContext == nil {
				return
			}
			invalidateError := options.NotifyViteFileMapChangedWithContext(
				invalidateContext,
			)
			...
			if options.Log != nil {
				options.Log.Warn(
					"vite filemap notify endpoint failed; falling back to hard reload",
					"error",
					invalidateError,
				)
			}
...
// NotifyViteFileMapChangedWithContext posts to Vite's filemap-changed notify
// endpoint with explicit cancellation context.
func NotifyViteFileMapChangedWithContext(
	...
	if response.StatusCode >= 400 {
		return fmt.Errorf(
			"vite filemap notify endpoint returned %d",
			response.StatusCode,
		)
	}
	return nil
...
func applyFrameworkRuntimeReloadRequestHeaders(
	...
	if trimmedReloadAttemptID != "" {
		request.Header.Set(
			waveframework.FrameworkRuntimeReloadAttemptIDHeaderName,
	...
	if trimmedReloadTrigger != "" {
		request.Header.Set(
			waveframework.FrameworkRuntimeReloadTriggerHeaderName,
