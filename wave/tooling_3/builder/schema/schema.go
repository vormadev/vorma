...
func (processor *Processor) WriteSchema() error {
	...
	if writeError := toolingshared.WriteFileAtomically(targetPath, schemaBytes, 0o644); writeError != nil {
		return writeError
	}
...
func (processor *Processor) BuildSchemaDocument() (map[string]any, error) {
	...
	if len(processor.cfg.FrameworkSchemaExtensions) > 0 {
		extensionsSchema, extensionError := processor.buildFrameworkExtensionSchema()
		if extensionError != nil {
			return nil, extensionError
		}
		properties["Framework"] = extensionsSchema
	}
...
// buildFrameworkExtensionSchema converts framework extension entries into raw schema map.
func (processor *Processor) buildFrameworkExtensionSchema() (map[string]any, error) {
	...
	return map[string]any{
		"type":                 "object",
		"additionalProperties": false,
		"properties":           serializedExtensionEntries,
	}, nil
}
