...
func ShouldShowRebuildingOverlayForEventsWithHooks(
	...
	for _, eventWithHooks := range eventsWithHooks {
		if eventWithHooks.Classified.WatchedFile != nil &&
			eventWithHooks.Classified.WatchedFile.SkipRebuildingNotification {
			continue
		}
		return true
	}
...
// DetermineBrowserBehavior derives browser behavior from current workset state.
func DetermineBrowserBehavior(
	work WorkSet,
	usingVite bool,
) BrowserPhaseDecision {
	_ = usingVite
...
func DeriveBrowserPhaseResolutionForWorkSet(
	...
	return BrowserPhaseResolution{
		Action:         workSet.Browser.Action,
		ApplyWaitFlags: true,
		WaitForApp:     workSet.Browser.WaitForApp,
		WaitForVite:    workSet.Browser.WaitForVite,
	}
}
...
func DeriveImplicitWorkDecisionForClassifiedEvent(
	...
	case FileTypePublicStatic:
		...
		decision.PublicStaticChangedFilePath = path
		decision.RestartApp = false

	case FileTypePrivateStatic:
		decision.ProcessPrivateFiles = true
		decision.PrivateStaticChangedFilePath = path
		decision.RestartApp = false
	}

	if classifiedEvent.WatchedFile != nil {
		if classifiedEvent.WatchedFile.RecompileGoBinary {
			decision.CompileGo = true
		}
		if classifiedEvent.WatchedFile.RestartApp {
			decision.RestartApp = true
		}
...
func ApplyImplicitWorkDecision(
	work *WorkSet,
	...
	work.Build.CompileGo = work.Build.CompileGo ||
		implicitWorkDecision.CompileGo
	work.Build.BuildCriticalCSS = work.Build.BuildCriticalCSS ||
		implicitWorkDecision.BuildCriticalCSS
	work.Build.BuildNormalCSS = work.Build.BuildNormalCSS ||
		implicitWorkDecision.BuildNormalCSS
	work.Build.ProcessPublicFiles = work.Build.ProcessPublicFiles ||
		implicitWorkDecision.ProcessPublicFiles
	work.Build.ProcessPrivateFiles = work.Build.ProcessPrivateFiles ||
		implicitWorkDecision.ProcessPrivateFiles
	work.Restart.RestartApp = work.Restart.RestartApp ||
		implicitWorkDecision.RestartApp
	work.PreferRevalidate = work.PreferRevalidate ||
		implicitWorkDecision.PreferRevalidate

	if implicitWorkDecision.PublicStaticChangedFilePath != "" {
		work.ensurePublicStaticChangedPathSet()
		if _, alreadyIncluded := work.Build.PublicStaticChangedFilePathSet[implicitWorkDecision.PublicStaticChangedFilePath]; !alreadyIncluded {
			work.Build.PublicStaticChangedFilePathSet[implicitWorkDecision.PublicStaticChangedFilePath] = struct{}{}
			work.Build.PublicStaticChangedFilePaths = append(
				work.Build.PublicStaticChangedFilePaths,
				implicitWorkDecision.PublicStaticChangedFilePath,
			)
		}
	}
	if implicitWorkDecision.PrivateStaticChangedFilePath != "" {
		work.ensurePrivateStaticChangedPathSet()
		if _, alreadyIncluded := work.Build.PrivateStaticChangedFilePathSet[implicitWorkDecision.PrivateStaticChangedFilePath]; !alreadyIncluded {
			work.Build.PrivateStaticChangedFilePathSet[implicitWorkDecision.PrivateStaticChangedFilePath] = struct{}{}
			work.Build.PrivateStaticChangedFilePaths = append(
...
func (work *WorkSet) AddImplicitWork(classifiedEvent ClassifiedEvent) {
	...
	ApplyImplicitWorkDecision(work, implicitWorkDecision)
}
...
func (work *WorkSet) AddFromRefreshAction(
	...
	return ApplyRefreshActionWorkMutationDecision(
		work,
...
func (work *WorkSet) Resolve(usingVite bool) {
	...
	if work.Restart.RestartApp {
		...
		work.Browser.WaitForVite = usingVite
		work.Browser.CycleVite = false
		return
	}

	if work.Build.BuildCriticalCSS || work.Build.BuildNormalCSS {
		work.Browser.Action = BrowserPhaseActionHotReloadCSS
		work.Browser.WaitForApp = false
		work.Browser.WaitForVite = false
		work.Browser.CycleVite = false
		return
	}

	if work.Build.ProcessPublicFiles || work.Build.ProcessPrivateFiles {
		if work.PreferRevalidate {
			work.Browser.Action = BrowserPhaseActionRevalidate
			work.Browser.WaitForApp = true
			work.Browser.WaitForVite = usingVite
			work.Browser.CycleVite = false
			return
	...
	work.Browser.Action = BrowserPhaseActionNone
}

// ensurePublicStaticChangedPathSet initializes public static set lazily.
func (work *WorkSet) ensurePublicStaticChangedPathSet() {
	if work.Build.PublicStaticChangedFilePathSet == nil {
		work.Build.PublicStaticChangedFilePathSet = make(map[string]struct{})
	}
}

// ensurePrivateStaticChangedPathSet initializes private static set lazily.
func (work *WorkSet) ensurePrivateStaticChangedPathSet() {
	if work.Build.PrivateStaticChangedFilePathSet == nil {
		work.Build.PrivateStaticChangedFilePathSet = make(map[string]struct{})
	}
}
...
func DeriveRefreshActionWorkMutationDecision(
	...
	if refreshAction.TriggerRestart {
		...
		decision.RequestBrowserAction = false
		return decision
	}

	if refreshAction.ReloadBrowser {
		decision.RequestBrowserAction = true
		if refreshAction.WaitForApp || refreshAction.WaitForVite {
			decision.BrowserAction = BrowserPhaseActionHardReload
		} else {
			decision.BrowserAction = BrowserPhaseActionRevalidate
		}
...
func ApplyRefreshActionWorkMutationDecision(
	work *WorkSet,
	...
	if mutationDecision.RestartApp {
		...
		work.Browser = BrowserPhaseDecision{}
		return applicationResult
	}

	if mutationDecision.RequestBrowserAction {
		work.Browser.Action = mutationDecision.BrowserAction
		work.Browser.WaitForApp = work.Browser.WaitForApp ||
			mutationDecision.WaitForApp
		work.Browser.WaitForVite = work.Browser.WaitForVite ||
			mutationDecision.WaitForVite
	}

	return applicationResult
}

// ReduceRefreshActionsInStableOrder applies actions in order until restart short-circuit.
func ReduceRefreshActionsInStableOrder(
	...
	for _, refreshAction := range refreshActions {
		...
		mutationDecision := DeriveRefreshActionWorkMutationDecision(
			refreshAction,
		)
		applicationResult := ApplyRefreshActionWorkMutationDecision(
			work,
			mutationDecision,
		)

		if applicationResult.RestartRequested {
			reductionDecision.ApplicationResult = applicationResult
...
func HasPathAlias(path string, aliases []string) bool {
	for _, alias := range aliases {
		if waveshared.PathsReferToSameLocation(path, alias) {
			return true
		}
	}
	return false
