...
func WriteConfigSchema(cfg *wave.ParsedConfig) error {
	schema := buildConfigSchemaRootEntry(cfg)
	...
	target := filepath.Join(cfg.Dist.Internal(), "schema.json")
...
func buildConfigSchemaRootEntry(
	...
	if len(cfg.FrameworkSchemaExtensions) == 0 {
		return schema
	}

	properties := schema.Properties.(map[string]jsonschema.Entry)
	maps.Copy(properties, cfg.FrameworkSchemaExtensions)
	return schema
}
