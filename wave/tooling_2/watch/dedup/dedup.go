...
func NormalizeWatcherEventPathForDeduplication(
	...
	cleanedEventPath := waveshared.TrimAndCleanPath(eventPath)
	if cleanedEventPath == "" {
		return ""
	}

	if IsAbsolutePathForWatcherEventDeduplication(cleanedEventPath) {
		return waveshared.Absolute(cleanedEventPath)
	}
...
func canonicalizeAbsolutePathForWatcherEventDeduplication(
	...
	return waveshared.CanonicalizePathForLocationComparison(absolutePath)
}
...
func MissingFileAliasKeyForWatcherEventDeduplication(
	...
	parentDirectoryPath := waveshared.AbsoluteDirectory(absolutePath)
