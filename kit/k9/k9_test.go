...
func TestNewMulti(t *testing.T) {
	...
	for i, id := range ids {
		...
		if !k9Pattern.MatchString(serialized) {
			t.Fatalf("NewMulti(8)[%d].Serialize() format = %q, want lowercase base32 (%d chars)", i, serialized, stringSize)
		}
		ts := id.ToUnixMicro()
		if i == 0 {
		...
		if ts != expectedTimestamp {
			t.Fatalf("NewMulti(8) should share one timestamp tick, ids[%d]=%d want %d", i, ts, expectedTimestamp)
		}
	}
...
func TestRoundTripForGeneratedIDs(t *testing.T) {
	for i := 0; i < 256; i++ {
		...
		if !k9Pattern.MatchString(encoded) {
			t.Fatalf("Serialize() format at iteration %d = %q, want lowercase base32 (%d chars)", i, encoded, stringSize)
		}
		...
		if decoded != id {
			t.Fatalf("Parse(Serialize(id)) mismatch at iteration %d: got=%x want=%x", i, decoded[:], id[:])
		}

		reEncoded := decoded.Serialize()
		if reEncoded != encoded {
			t.Fatalf("Serialize(Parse(Serialize(id))) mismatch at iteration %d: got=%q want=%q", i, reEncoded, encoded)
		}
	}
}
...
func TestParseIsCaseInsensitive(t *testing.T) {
	...
	if upperDecoded != lowerDecoded {
		t.Fatalf("Parse() should decode uppercase/lowercase the same: upper=%x lower=%x", upperDecoded[:], lowerDecoded[:])
	}
}
