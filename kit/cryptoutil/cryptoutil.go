...
func VerifyAndReadAsymmetric(
	...
	ok := ed25519.Verify(
		publicKey[:],
		signedMsg[ed25519.SignatureSize:],
		signedMsg[:ed25519.SignatureSize],
	)
	if !ok {
		return nil, errInvalidSignature
...
func ValidateHmacSha256(
	attemptedMsg, attemptedKey, knownGoodMAC []byte,
) (bool, error) {
	if attemptedKey == nil {
...
func DecryptSymmetricXChaCha20Poly1305(
	...
	return DecryptSymmetricGeneric(
		ToAEADFuncXChaCha20Poly1305,
		encryptedMsg,
		secretKey,
	)
}
