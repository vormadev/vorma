...
Task-local context for `__TICKET.md`. This file records the response/error protocol design
discussion that fed the API pass; it is not the live plan. The current response model
lives in code, tests, `API_DESIGN.md`, and `docs/maintainer/ARCHITECTURE.md`.

Rulings already settled by the user that the API design pass MUST honor:
...
- Redirects: handle-based leaning, BUT the fabricated-return wart (handler must still
  ...
  `return ctx.redirect(...)` shape and exit-type naming were open at this point in the
  design pass.

Current reconciliation, 2026-06-23: this document records the pre-landing design
discussion. The open choices below were resolved and landed through the API design pass.
Current code uses `ViewExit` / `HttpExit`, explicit client messages, JSON error envelopes
for resource errors, `ctx.redirect(...)` riding the exit channel, no view status surface,
and resource success statuses only through the resource response handle. Treat the "Open
rulings" section below as historical deliberation, not current work.
...
## Historical Open Rulings, Resolved Later
