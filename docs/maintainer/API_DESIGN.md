...
Status: IN REVIEW. Full-surface pass (not example-driven) over every public item in the
Rust crates and the TS package, evaluated against the ruled principles. Census inputs:
...
1. ✅ DONE — Error/exit protocol, landed end-to-end and e2e-proven:
    ...
    - Remaining statusText sites: create_client_core's route-fetch error path
      (view-navigation faults) still reads statusText — joins the envelope in the next
      stage alongside config reshape.
...
4. **Example expansion (user-ratified; IN PROGRESS): the example grows to exercise the
   FULL API surface** — the "minimal" framing goes away (rename to examples/notes, "Vorma
   Notes"). Coverage contract (each line must be exercised by the finished example, with
   the awkwardness it surfaces logged here):
    ...
    RUST SLICE LANDED (client components + TS breadth still pending). Workout findings so
    far:
    ...
    TS SLICE LANDED (full pipeline: tsgen + vite build green; tsgo/oxlint clean). Client
    now exercises: nested Outlet under a layout view, clientLoader + serverPromise,
    errorBoundary + defaultErrorBoundary, beforeRouteCommit, useRouteSync (debounced
    ?draft= sync), useViewData/useClientLoaderData/useRouteState/useWorkState (work
    indicator), explicit prefetch/cancelPrefetch alongside linkDefaultProps
    prefetch:"intent", revalidateOnWindowFocus, apiClient mutateOrThrow (JSON + FormData +
    DELETE + redirect-following) and queryOrThrow, kit/theme + kit/cookies, and the
    extra_types/export_const pipeline (`keyboardShortcuts` consumed from vorma.gen.ts).
...
5. Re-run the full gate + e2e matrix; update ARCHITECTURE.md and CURRENT_PLAN_3
   dispositions; THEN user-facing docs unblock.
