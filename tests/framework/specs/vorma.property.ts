import { actions, always, eventually, extract, now } from "@antithesishq/bombadil";
...
type FixtureState = {
	...
	probe_ready: boolean;
	rendered_view: string | null;
	counter_value_text: string;
	count_resource_next_text: string;
	...
	last_build_skew_resource_kind: string | null;
...
const fixture_state = extract((state): FixtureState => {
	...
	const public_css_url_probe_background_image =
		public_css_url_probe === null
			? ""
			: state.window.getComputedStyle(public_css_url_probe).backgroundImage;
	...
	return {
		is_dev: data_payload?.is_dev === true,
		...
		client_build_tag:
			...
			"",
		switch_result:
			state.document.querySelector("[data-bmb-switch-result]")?.textContent ?? "",
		expected_deployment:
		...
		search_delay_ms: Number.isFinite(search_delay_ms) ? search_delay_ms : null,
		current_href_text:
		...
		rendered_view:
			state.document
				.querySelector("[data-bmb-view]")
				?.getAttribute("data-bmb-view") ?? null,
		counter_value_text:
			state.document.querySelector("[data-bmb-counter-value]")?.textContent ?? "",
		count_resource_next_text:
			state.document.querySelector("[data-bmb-count-resource-next]")?.textContent ??
			"",
		slow_delay_text:
			state.document.querySelector("[data-bmb-slow-delay]")?.textContent ?? "",
		item_id:
			state.document
		...
		client_server_id_text:
			state.document.querySelector("[data-bmb-client-server-id]")?.textContent ??
			"",
		client_server_stamp_text:
			state.document.querySelector("[data-bmb-client-server-stamp]")?.textContent ??
			"",
		client_loader_id_text:
			state.document.querySelector("[data-bmb-client-loader-id]")?.textContent ??
			"",
		client_loader_trigger_text:
			state.document.querySelector("[data-bmb-client-loader-trigger]")
				?.textContent ?? "",
		client_loader_stamp_text:
			state.document.querySelector("[data-bmb-client-loader-stamp]")?.textContent ??
			"",
		echo_operation:
			state.document.querySelector("[data-bmb-echo-operation]")?.textContent ?? "",
		nested_layout_count: state.document.querySelectorAll("[data-bmb-nested-layout]")
			.length,
		nested_section:
			state.document
		...
		nested_detail_section_text:
			state.document.querySelector("[data-bmb-nested-detail-section]")
				?.textContent ?? "",
		echo_output:
			state.document.querySelector("[data-bmb-echo-output]")?.textContent ?? "",
		form_output:
			state.document.querySelector("[data-bmb-form-output]")?.textContent ?? "",
		error_text: state.document.querySelector("[data-bmb-error]")?.textContent ?? "",
		...
		last_build_skew_resource_kind: probe?.last_build_skew?.resource_kind ?? null,
		...
		public_css_url_probe_background_image: public_css_url_probe_background_image,
	};
});
...
function is_settled_route() {
	return (
		...
		fixture_state.current.probe_href === fixture_state.current.location_href &&
		fixture_state.current.current_href_text === fixture_state.current.location_href
	);
}
...
function api_skew_kept_client_build_id(resource_kind: string, ok: boolean) {
	return (
		...
		fixture_state.current.last_api_build_skew_resource_kind === resource_kind &&
...
export const vorma_rendered_view_matches_probe = always(() => {
	if (!fixture_state.current.probe_ready) {
		return true;
	}
	const probe_href = fixture_state.current.probe_href;
...
export const vorma_rendered_view_never_matches_pending_navigation = always(() => {
	if (!fixture_state.current.probe_ready || fixture_state.current.pending_href === "") {
		return true;
	}
	return fixture_state.current.current_href_text !== fixture_state.current.pending_href;
});

export const vorma_navigation_timing_is_ordered = always(() => {
	const pending_to_route = fixture_state.current.last_navigation_pending_to_route_ms;
	const pending_to_clear = fixture_state.current.last_navigation_pending_to_clear_ms;
	const pending_to_dom = fixture_state.current.last_navigation_pending_to_dom_ms;
	const route_to_dom = fixture_state.current.last_navigation_route_to_dom_ms;
	const route_to_clear = fixture_state.current.last_navigation_route_to_clear_ms;
	return (
...
export const vorma_resolves_public_urls_in_imported_css = always(() => {
	...
	const background_image = fixture_state.current.public_css_url_probe_background_image;
	return (
...
export const vorma_deployment_marker_is_known = always(() => {
	...
	return (
		fixture_state.current.view_deployment === "from-A" ||
		fixture_state.current.view_deployment === "from-B"
	);
});
...
export const vorma_switch_result_is_known = always(() => {
	return (
		...
		fixture_state.current.switch_result === "revalidated:max_retries_exhausted" ||
...
export const vorma_build_skew_events_include_context = always(() => {
	...
	return (
		...
		fixture_state.current.last_build_skew_current_work_submission_count !== null
	);
});

export const vorma_switch_revalidation_detects_build_skew = always(
	now(() => {
		return fixture_state.current.expected_operation === "revalidate-pending";
	}).implies(
		eventually(() => {
			return (
				fixture_state.current.manual_revalidation_build_skew_detections > 0 &&
				fixture_state.current.expected_operation === "revalidate-build_skew" &&
				fixture_state.current.switch_result === "revalidated:build_skew"
			);
		}).within(5, "seconds"),
	),
...
export const vorma_switch_navigation_reaches_new_deployment = always(
	now(() => {
		...
		eventually(() => {
			return (
				is_settled_route() &&
				fixture_state.current.view_deployment ===
...
export const vorma_switch_failed_mutation_detects_api_skew_without_adoption = always(
	now(() => {
		return (
			fixture_state.current.expected_operation === "mutation-error-pending" &&
			(fixture_state.current.expected_deployment === "A" ||
				fixture_state.current.expected_deployment === "B")
		);
	}).implies(
		eventually(() => {
			return (
				fixture_state.current.expected_operation === "mutation-error-error" &&
				fixture_state.current.failed_api_build_skew_detections > 0 &&
...
export const vorma_home_view_renders_home = always(() => {
	if (!is_settled_route() || fixture_state.current.pathname !== "/") {
		return true;
	}
	return fixture_state.current.rendered_view === "home";
});

export const vorma_counter_view_data_matches_query_value = always(() => {
	...
	return (
		fixture_state.current.rendered_view === "counter" &&
		fixture_state.current.counter_value_text === expected
	);
});

export const vorma_count_query_resource_returns_clamped_value = always(() => {
	if (
		!is_settled_route() ||
		fixture_state.current.pathname !== "/counter" ||
		fixture_state.current.count_resource_next_text === ""
	) {
		return true;
	}
	return (
		fixture_state.current.count_resource_next_text === "-5" ||
		fixture_state.current.count_resource_next_text === "5"
	);
});

export const vorma_slow_view_data_matches_delay = always(() => {
	...
	return (
		fixture_state.current.rendered_view === "slow" &&
		fixture_state.current.slow_delay_text === expected
	);
});

export const vorma_item_view_matches_url_param = always(() => {
	if (!is_settled_route() || !fixture_state.current.pathname.startsWith("/items/")) {
		return true;
	}
	const expected = fixture_state.current.pathname.slice("/items/".length);
	return (
		fixture_state.current.rendered_view === "item" &&
		fixture_state.current.item_id === expected
	);
});

export const vorma_client_view_matches_url_param = always(() => {
	if (!is_settled_route() || !fixture_state.current.pathname.startsWith("/client/")) {
		return true;
	}
	const expected = fixture_state.current.pathname.slice("/client/".length);
	return (
		fixture_state.current.rendered_view === "client" &&
...
export const vorma_client_view_has_client_loader_data = always(() => {
	if (!is_settled_route() || !fixture_state.current.pathname.startsWith("/client/")) {
		return true;
	}
	return (
...
export const vorma_nested_index_renders_layout = always(() => {
	...
	return (
		fixture_state.current.rendered_view === "nested-index" &&
...
export const vorma_nested_detail_renders_parent_and_leaf = always(() => {
	...
	return (
		fixture_state.current.rendered_view === "nested-detail" &&
		fixture_state.current.nested_layout_count === 1 &&
		fixture_state.current.nested_section === "nested" &&
...
export const vorma_echo_view_has_output = always(() => {
	if (!is_settled_route() || fixture_state.current.pathname !== "/echo") {
		return true;
	}
	return (
		fixture_state.current.rendered_view === "echo" &&
		fixture_state.current.echo_output.trim().length > 0
	);
});

export const vorma_fail_view_uses_error_boundary = always(() => {
	if (!is_settled_route() || fixture_state.current.pathname !== "/fail") {
		return true;
	}
	return (
		fixture_state.current.rendered_view === "fail" &&
...
export const vorma_form_submit_eventually_succeeds = always(
	now(() => {
		return fixture_state.current.form_output === "form-pending";
	}).implies(
		eventually(() => {
			return fixture_state.current.form_output === "accepted";
		}).within(5, "seconds"),
	),
);
...
export const vorma_fixture_actions = actions(() => {
	...
	if (
		...
		const count_resource_targets = click_targets.current.filter((target) => {
			return target.name.startsWith("count-resource-");
		});
		if (count_resource_targets.length > 0) {
			return count_resource_targets.map((target) => {
				return {
					Click: {
						name: target.name,
	...
	return [
		...
		...(fixture_state.current.history_back_count > 0 ? ["Back" as const] : []),
		...(fixture_state.current.history_forward_count > 0 ? ["Forward" as const] : []),
