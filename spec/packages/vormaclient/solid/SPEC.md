...
- `VORMACLIENT-SOLID-001` Entry-point export surface.
  ...
  `location`).
- `VORMACLIENT-SOLID-002` Typed route alias surface.
...
- `VORMACLIENT-SOLID-005` Link anchor wiring contract.
...
- `VORMACLIENT-SOLID-007` Root-outlet/location adapter surface contract.
  Package MUST expose Solid root-outlet and location signal surfaces. Rendering
  ...
  contract.
- `VORMACLIENT-SOLID-008` TypeScript JSX config contract.
  `tsconfig.json` MUST extend repo base config and set Solid JSX settings
