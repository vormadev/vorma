...
- `VORMACLIENT-REACT-001` Entry-point export surface. `index.tsx` MUST export
  ...
  `useLocation`).
- `VORMACLIENT-REACT-002` Typed route alias surface. Helper-level
...
- `VORMACLIENT-REACT-005` Link anchor wiring contract. `VormaLink` MUST delegate
...
- `VORMACLIENT-REACT-006` Typed link destination-resolution contract.
  ...
  `VormaLink`.
- `VORMACLIENT-REACT-007` Root-outlet/location adapter surface contract. Package
  MUST expose React root-outlet and `useLocation` surfaces. Rendering and
  ...
  contract.
- `VORMACLIENT-REACT-008` TypeScript JSX config contract. `tsconfig.json` MUST
  extend repo base config and set React JSX mode (`jsx: react-jsx`).
