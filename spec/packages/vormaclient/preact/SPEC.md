...
root-outlet adapter surface over owner runtime behavior in
`vormaclient/client`.
...
- `VORMACLIENT-PREACT-001` Entry-point export surface.
  ...
  `location`).
- `VORMACLIENT-PREACT-002` Typed route alias surface.
...
- `VORMACLIENT-PREACT-005` Link anchor wiring contract.
...
- `VORMACLIENT-PREACT-007` Root-outlet/location adapter surface contract.
  ...
  contract.
- `VORMACLIENT-PREACT-008` TypeScript JSX config contract.
  `tsconfig.json` MUST extend repo base config and set Preact JSX settings
