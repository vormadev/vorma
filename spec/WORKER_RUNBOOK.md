...
2. Claim work from `spec/MINING_DISPATCH.md` using tooling, not manual row
   edits.
3. Edit only one claimed package path under `spec/packages/**`.
4. Every requirement in `20-requirements.md` must map in `evidence.yaml`.
5. Every mapped requirement must include both test and implementation evidence.
6. `80-assertion-accounting.md` must satisfy:
    - `mapped_meaningful_assertions = meaningful_assertions`
    - `unclassified_assertions = 0`
7. Do not mark a slot `DONE` unless guard checks pass.

## Required Read Order

1. `AGENTS.md`
2. `spec/PROGRAM.md`
3. `spec/ASSERTION_ACCOUNTING.md`
4. `spec/CHECKLIST.md`
5. `spec/MINING_DISPATCH.md`
...
2. Find your claimed slot row in `spec/MINING_DISPATCH.md` and note `slot_id`
   and `spec_path`.

3. Edit only files under your claimed `spec_path`.
...
5. If checks pass, mark your slot done:

```bash
spec/tools/mark_slot_done.sh SLOT-XXX
```
