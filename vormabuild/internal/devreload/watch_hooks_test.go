...
func TestDefaultWatchPatternCallbacks_RoutesAndTemplate(t *testing.T) {
	...
	t.Run("routes callback app-stopped returns nil action", func(t *testing.T) {
		action, err := routesHook(&wavewatch.HookContext{AppStoppedForBatch: true})
		if err != nil {
			t.Fatalf("routes callback returned error: %v", err)
		}
		if action != nil {
	...
	t.Run(
		...
		func(t *testing.T) {
			...
			templateHookContext := &wavewatch.HookContext{AppStoppedForBatch: false}
	...
	t.Run(
		...
		func(t *testing.T) {
			action, err := templateHook(
				&wavewatch.HookContext{AppStoppedForBatch: true},
			)
			if err != nil {
				t.Fatalf("template callback returned error: %v", err)
			}
