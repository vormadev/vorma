...
func (builder *fakeRuntimeWaveBuilder) Build(
...
func TestRunWaveViteProductionBuild(t *testing.T) {
	...
	t.Run(
		"propagates Vite build error and still closes builder",
		func(t *testing.T) {
			fixture := testkit.NewBuildTestFixture(t, nil)
			app := fixture.App
			...
			err := runWaveViteProductionBuildWithToolingDependencies(
				app,
				dependencies,
			)
			if err == nil {
			...
			if !builder.closeCalled {
				t.Fatal(
					"expected builder.Close to be called after Vite build error",
				)
			}
		},
	)
	...
	t.Run("joins close error when Vite build fails", func(t *testing.T) {
		...
		if err == nil {
			t.Fatal(
				"expected runWaveViteProductionBuild to return joined build+close error",
			)
		}
...
func TestRunWaveDevelopmentServer(t *testing.T) {
	...
	dependencies := runtimeBuildToolingDependencies{
		runWaveDevelopmentMode: func(v *vormaruntime.Vorma) error {
			...
			if v != app {
				t.Fatalf(
					"runWaveDevelopmentMode received app %p, want %p",
					v,
					app,
				)
			}
...
func TestRunWaveProductionBuild(t *testing.T) {
	t.Run(
		"runs production build with options and closes builder",
		func(t *testing.T) {
			fixture := testkit.NewBuildTestFixture(t, nil)
			app := fixture.App
			...
			if builder.receivedOption != buildOptions {
				t.Fatalf(
					"builder.Build received options %#v, want %#v",
					builder.receivedOption,
					buildOptions,
				)
			}
			if !builder.closeCalled {
	...
	t.Run(
		"propagates production build error and still closes builder",
		func(t *testing.T) {
			fixture := testkit.NewBuildTestFixture(t, nil)
			app := fixture.App
			...
			err := runWaveProductionBuildWithToolingDependencies(
				app,
				productionBuildOptions(false),
				dependencies,
			)
			if err == nil {
			...
			if !errors.Is(err, expectedErr) {
				t.Fatalf(
					"error = %v, expected wrapped production build error",
					err,
				)
			}
			if !builder.closeCalled {
	...
	t.Run(
		"returns close error when production build succeeds",
		func(t *testing.T) {
			fixture := testkit.NewBuildTestFixture(t, nil)
			app := fixture.App
	...
	t.Run("joins close error when production build fails", func(t *testing.T) {
		...
		if err == nil {
			t.Fatal(
				"expected runWaveProductionBuild to return joined build+close error",
			)
		}
		if !errors.Is(err, expectedBuildErr) {
			t.Fatalf("error = %v, expected build error in joined chain", err)
...
func TestRuntimeBuildToolingDefaultSteps(t *testing.T) {
	...
	if err == nil {
		t.Fatal(
			"expected default runWaveDevelopmentMode to return config validation error",
		)
	}
...
func TestRunProdHookPostProcessing(t *testing.T) {
	t.Run("runs vite build then post-processing", func(t *testing.T) {
		...
		err := runProdHookPostProcessingWithRuntimeBuildDependencies(
			app,
			dependencies,
		)
		if err != nil {
...
func TestRuntimeBuildExecutorRunDevelopmentMode(t *testing.T) {
	...
	if err := newRuntimeBuildExecutorWithDependencies(app, dependencies).runDevelopmentMode(); err != nil {
		t.Fatalf(
			"runtimeBuildExecutor.runDevelopmentMode returned error: %v",
			err,
		)
	}
...
func TestRuntimeBuildExecutorRunProductionMode(t *testing.T) {
	t.Run("passes expected build options", func(t *testing.T) {
		...
		if err := newRuntimeBuildExecutorWithDependencies(app, dependencies).runProductionMode(true); err != nil {
			t.Fatalf(
				"runtimeBuildExecutor.runProductionMode returned error: %v",
				err,
			)
		}
		if capturedOptions.CompileGo {
			t.Fatalf(
				"CompileGo = %v, want false when noBinary=true",
				capturedOptions.CompileGo,
			)
		}
	...
	t.Run("propagates wave production build errors", func(t *testing.T) {
		...
		err := newRuntimeBuildExecutorWithDependencies(
			app,
			dependencies,
		).runProductionMode(false)
		if err == nil {
			t.Fatal(
				"expected runtimeBuildExecutor.runProductionMode to return error",
			)
		}
		if !errors.Is(err, expectedErr) {
...
func TestProductionBuildOptions(t *testing.T) {
	...
	if !optionsWithBinary.CompileGo {
		t.Fatalf(
			"CompileGo = %v, want true when noBinary=false",
			optionsWithBinary.CompileGo,
		)
	}
	...
	if !optionsWithBinary.SkipPostHookPublicStaticProcessing {
		t.Fatalf(
			"SkipPostHookPublicStaticProcessing = %v, want true",
			optionsWithBinary.SkipPostHookPublicStaticProcessing,
		)
	}
	...
	if optionsWithoutBinary.CompileGo {
		t.Fatalf(
			"CompileGo = %v, want false when noBinary=true",
			optionsWithoutBinary.CompileGo,
		)
	}
	...
	if !optionsWithoutBinary.SkipPostHookPublicStaticProcessing {
		t.Fatalf(
			"SkipPostHookPublicStaticProcessing = %v, want true",
			optionsWithoutBinary.SkipPostHookPublicStaticProcessing,
		)
	}
}
...
func TestBuild(t *testing.T) {
	t.Run(
		"dev mode configures environment and runs dev build path",
		func(t *testing.T) {
			fixture := testkit.NewBuildTestFixture(t, nil)
			app := fixture.App
	...
	t.Run("production mode returns production build error", func(t *testing.T) {
		...
		err := buildWithRuntimeBuildDependencies(
			app,
			false,
			false,
			dependencies,
