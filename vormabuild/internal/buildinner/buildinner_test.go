...
type fakeBuildInnerPublicFileMapWriter struct {
	processPublicFilesErr        error
	writeErr                     error
	closeErr                     error
	processPublicFilesOnlyCalled bool
	writeCalled                  bool
	closeCalled                  bool
...
func (writer *fakeBuildInnerPublicFileMapWriter) ProcessPublicFilesOnly() error {
	writer.processPublicFilesOnlyCalled = true
	writer.callOrder = append(writer.callOrder, "process")
	return writer.processPublicFilesErr
}

func (writer *fakeBuildInnerPublicFileMapWriter) WritePublicFileMapTS(
	outDir string,
) error {
	writer.writeCalled = true
	writer.receivedOutDir = outDir
	writer.callOrder = append(writer.callOrder, "write")
	return writer.writeErr
}

func (writer *fakeBuildInnerPublicFileMapWriter) Close() error {
	writer.closeCalled = true
	writer.callOrder = append(writer.callOrder, "close")
...
func TestBuildInner(t *testing.T) {
	...
	t.Run("runs all build steps in order", func(t *testing.T) {
		...
		if len(observedStepOrder) != len(expectedStepOrder) {
			t.Fatalf(
				"observed step order length = %d, want %d (%#v)",
				len(observedStepOrder),
				len(expectedStepOrder),
				observedStepOrder,
			)
		}
	...
	t.Run(
		"runs non-commit build steps without holding runtime write lock",
		func(t *testing.T) {
	...
	t.Run(
		"restores runtime state snapshot when a step fails",
		func(t *testing.T) {
			...
			for _, failureCase := range failureCases {
				...
				t.Run(failureCase.name, func(t *testing.T) {
					...
					mutateRuntimeStateForBuildInnerFailure := func(
						...
						v.WithLock(func(l *vormaruntime.LockedVorma) {
							...
							l.SetRouteManifestFile(
								"route-manifest-after-" + stateID + ".json",
							)
					...
					dependencies.writePublicFileMapTypeScript = func(v *vormaruntime.Vorma) error {
						mutateRuntimeStateForBuildInnerFailure(
							v,
							"write-public-file-map",
						)
					...
					dependencies.logBuildInnerCompletion = func(*vormaruntime.Vorma, time.Time) {
						t.Fatal(
							"did not expect completion log when buildInner step fails",
						)
					}
					...
					if failureCase.expectedErrorText != "" &&
						!strings.Contains(
							err.Error(),
							failureCase.expectedErrorText,
						) {
						t.Fatalf(
							"error = %q, expected %q context",
							err,
							failureCase.expectedErrorText,
						)
					}
					...
					if app.IsDevMode() {
						t.Fatal(
							"expected runtime state rollback to restore isDev=false",
						)
					}
					if got := app.BuildID(); got != baselineBuildID {
						t.Fatalf(
							"build ID after rollback = %q, want %q",
							got,
							baselineBuildID,
						)
					}
					...
					if len(paths) != 1 {
						t.Fatalf(
							"paths after rollback length = %d, want 1 (%#v)",
							len(paths),
							paths,
						)
					}
					if paths["/existing"] == nil {
						t.Fatalf(
							"expected /existing path after rollback, got %#v",
							paths,
						)
					}
				})
	...
	t.Run(
		"restores runtime state snapshot then re-panics when a step panics",
		func(t *testing.T) {
			fixture := testkit.NewBuildTestFixture(t, nil)
			app := fixture.App
			...
			defer func() {
				...
				if got := app.BuildID(); got != baselineBuildID {
					t.Fatalf(
						"build ID after panic rollback = %q, want %q",
						got,
						baselineBuildID,
					)
				}
				...
				if len(paths) != 1 {
					t.Fatalf(
						"paths after panic rollback length = %d, want 1 (%#v)",
						len(paths),
						paths,
					)
				}
				if paths["/existing"] == nil {
					t.Fatalf(
						"expected /existing path after panic rollback, got %#v",
						paths,
					)
				}
			}()
	...
	t.Run(
		"skips rollback and re-panics when build ID token is superseded before panic rollback",
		func(t *testing.T) {
			fixture := testkit.NewBuildTestFixture(t, nil)
			app := fixture.App
			...
			expectedPanic := errors.New(
				"parse panic after newer build committed",
			)
			dependencies := newDependencies()
			...
			defer func() {
				...
				if !app.IsDevMode() {
					t.Fatal(
						"expected newer runtime state to remain in dev mode after stale panic rollback skip",
					)
				}
				if got := app.BuildID(); got != "build-after-newer-sync" {
					t.Fatalf(
						"build ID after stale panic rollback skip = %q, want %q",
						got,
						"build-after-newer-sync",
					)
				}
				...
				if len(paths) != 1 {
					t.Fatalf(
						"paths after stale panic rollback skip length = %d, want 1 (%#v)",
						len(paths),
						paths,
					)
				}
				if paths["/newer"] == nil {
					t.Fatalf(
						"expected newer path to remain after stale panic rollback skip, got %#v",
						paths,
					)
				}
			}()
	...
	t.Run(
		"skips rollback when build ID token is superseded before failure rollback",
		func(t *testing.T) {
			fixture := testkit.NewBuildTestFixture(t, nil)
			app := fixture.App
			...
			expectedErr := errors.New(
				"parse failed after newer build committed",
			)
			dependencies := newDependencies()
			dependencies.getCurrentBuildIDWithReadLock = func(*vormaruntime.Vorma) string {
				return "build-attempt"
			...
			dependencies.writePublicFileMapTypeScript = func(*vormaruntime.Vorma) error {
				t.Fatal(
					"did not expect public file map write after parse failure",
				)
				return nil
			}
			dependencies.writeRouteArtifacts = func(*vormaruntime.Vorma) error {
				t.Fatal(
					"did not expect route artifact write after parse failure",
				)
				return nil
			}
			dependencies.logBuildInnerCompletion = func(*vormaruntime.Vorma, time.Time) {
			...
			if !app.IsDevMode() {
				t.Fatal(
					"expected newer runtime state to remain in dev mode after stale rollback skip",
				)
			}
			if got := app.BuildID(); got != "build-after-newer-sync" {
				t.Fatalf(
					"build ID after stale rollback skip = %q, want %q",
					got,
					"build-after-newer-sync",
				)
			}
			...
			if len(paths) != 1 {
				t.Fatalf(
					"paths after stale rollback skip length = %d, want 1 (%#v)",
					len(paths),
					paths,
				)
			}
			if paths["/newer"] == nil {
				t.Fatalf(
					"expected newer path to remain after stale rollback skip, got %#v",
					paths,
				)
			}
		},
	...
	t.Run(
		"preserves newer runtime state when overlapping build attempts race",
		func(t *testing.T) {
			fixture := testkit.NewBuildTestFixture(t, nil)
			app := fixture.App
			...
			firstBuildDependencies.cleanStaticPublicOutDir = func(*vormaruntime.Vorma) error {
				t.Fatal(
					"did not expect clean step in first build after parse failure",
				)
				return nil
			}
			firstBuildDependencies.writePublicFileMapTypeScript = func(*vormaruntime.Vorma) error {
				t.Fatal(
					"did not expect write public file map step in first build after parse failure",
				)
				return nil
			}
			firstBuildDependencies.writeRouteArtifacts = func(*vormaruntime.Vorma) error {
				t.Fatal(
					"did not expect write route artifacts step in first build after parse failure",
				)
				return nil
			}
			...
			case err := <-firstBuildErrCh:
				if err == nil {
					t.Fatal(
						"expected first concurrent buildInner to return parse error",
					)
				}
				if !strings.Contains(err.Error(), "parse client routes") {
					t.Fatalf(
						"first concurrent build error = %q, expected parse context",
						err,
					)
				}
				if !errors.Is(err, firstBuildExpectedErr) {
					t.Fatalf(
						"first concurrent build error = %v, expected wrapped parse error",
						err,
					)
				}
			case <-time.After(time.Second):
				t.Fatal(
					"timed out waiting for first concurrent build to return",
				)
			}

			if !app.IsDevMode() {
				t.Fatal(
					"expected newer runtime state from second build to remain in dev mode",
				)
			}
			if got := app.BuildID(); got != "build-second-attempt" {
				t.Fatalf(
					"build ID after overlapping builds = %q, want %q",
					got,
					"build-second-attempt",
				)
			}
			...
			if len(paths) != 1 {
				t.Fatalf(
					"paths after overlapping builds length = %d, want 1 (%#v)",
					len(paths),
					paths,
				)
			}
			if paths["/second"] == nil {
				t.Fatalf(
					"expected second-build path to remain after overlapping builds, got %#v",
					paths,
				)
			}
		},
	...
	t.Run("returns initialization error directly", func(t *testing.T) {
		...
		dependencies.parseAndSyncClientRoutes = func(*vormaruntime.Vorma) error {
			t.Fatal(
				"did not expect dependencies.parseAndSyncClientRoutes after initialization error",
			)
			return nil
		}
	...
	t.Run("wraps clean static public out dir errors", func(t *testing.T) {
		...
		if !errors.Is(err, expectedErr) {
			t.Fatalf(
				"error = %v, expected wrapped clean-static-public error",
				err,
			)
		}
	})
	...
	t.Run("wraps write public file map TS errors", func(t *testing.T) {
		...
		if !strings.Contains(err.Error(), "write public file map TS") {
			t.Fatalf(
				"error = %q, expected write-public-file-map-ts context",
				err,
			)
		}
		if !errors.Is(err, expectedErr) {
			t.Fatalf(
				"error = %v, expected wrapped write-public-file-map-ts error",
				err,
			)
		}
	})
	...
	t.Run("wraps write route artifacts errors", func(t *testing.T) {
		...
		if !errors.Is(err, expectedErr) {
			t.Fatalf(
				"error = %v, expected wrapped write-route-artifacts error",
				err,
			)
		}
	})
...
func TestShouldRollbackBuildInnerRuntimeStateAfterFailure(t *testing.T) {
	...
	if !shouldRollbackBuildInnerRuntimeStateAfterFailure(
		...
		t.Fatal(
			"expected rollback when current build ID matches attempt build ID token",
		)
	}
	if shouldRollbackBuildInnerRuntimeStateAfterFailure(
		"build-current",
		...
		t.Fatal(
			"expected rollback to skip when current build ID differs from attempt build ID token",
		)
	}
}
...
func TestWritePublicFileMapTypeScript(t *testing.T) {
	t.Run("writes TS into configured output directory", func(t *testing.T) {
		...
		if !writer.processPublicFilesOnlyCalled {
			t.Fatal("expected ProcessPublicFilesOnly to be called")
		}
		...
		expectedOrder := []string{"process", "write", "close"}
		if len(writer.callOrder) != len(expectedOrder) {
			t.Fatalf("call order = %#v, want %#v", writer.callOrder, expectedOrder)
		}
		for i := range expectedOrder {
			if writer.callOrder[i] != expectedOrder[i] {
				t.Fatalf("call order = %#v, want %#v", writer.callOrder, expectedOrder)
			}
		}
	})

	t.Run("returns process error and still closes writer", func(t *testing.T) {
		fixture := testkit.NewBuildTestFixture(t, nil)
		app := fixture.App

		expectedErr := errors.New("process failed")
		writer := &fakeBuildInnerPublicFileMapWriter{
			processPublicFilesErr: expectedErr,
		}
		dependencies := buildInnerPublicFileMapDependencies{
		...
		if err == nil {
			t.Fatal(
				"expected writePublicFileMapTypeScript to return process error",
			)
		}
		if !errors.Is(err, expectedErr) {
			t.Fatalf("error = %v, expected wrapped process error", err)
		}
		if writer.writeCalled {
			t.Fatal("expected WritePublicFileMapTS not to be called after process error")
		}
		if !writer.closeCalled {
			t.Fatal("expected writer.Close to be called after process error")
		}
	})
	...
	t.Run("returns write error and still closes writer", func(t *testing.T) {
		...
		if err == nil {
			t.Fatal(
				"expected writePublicFileMapTypeScript to return write error",
			)
		}
		if !errors.Is(err, expectedErr) {
	...
	t.Run("returns close error when write succeeds", func(t *testing.T) {
		...
		if err == nil {
			t.Fatal(
				"expected writePublicFileMapTypeScript to return close error",
			)
		}
		if !errors.Is(err, expectedErr) {
	...
	t.Run("joins close error when write fails", func(t *testing.T) {
		...
		if err == nil {
			t.Fatal(
				"expected writePublicFileMapTypeScript to return joined write+close error",
			)
		}
...
func TestParseAndSyncClientRoutes(t *testing.T) {
	t.Run("returns parse error", func(t *testing.T) {
		...
		err := parseAndSyncClientRoutesWithDependencies(
			&vormaruntime.Vorma{},
			dependencies,
		)
		if err == nil {
...
func TestInitializeBuildInnerState(t *testing.T) {
	t.Run(
		"production mode marks app as non-dev and skips build ID generation",
		func(t *testing.T) {
			fixture := testkit.NewBuildTestFixture(t, nil)
			app := fixture.App
			...
			dependencies := buildInnerBuildIDDependencies{
				generateDevBuildIDSuffix: func() (string, error) {
					t.Fatal(
						"did not expect dev build ID generation in production mode",
					)
					return "", nil
				},
			}
			...
			if app.IsDevMode() {
				t.Fatal(
					"expected app not to be in dev mode after production initialization",
				)
			}
	...
	t.Run(
		"development mode sets prefixed build ID from generated suffix",
		func(t *testing.T) {
			fixture := testkit.NewBuildTestFixture(t, nil)
			app := fixture.App
			...
			if !app.IsDevMode() {
				t.Fatal(
					"expected app to be in dev mode after development initialization",
				)
			}
	...
	t.Run(
		"development mode does not expose mixed runtime state while build ID is pending",
		func(t *testing.T) {
			fixture := testkit.NewBuildTestFixture(t, nil)
			app := fixture.App
			...
			if app.IsDevMode() {
				t.Fatal(
					"expected runtime to keep previous isDev value until build state commits atomically",
				)
			}
			...
			case err := <-initializeErrCh:
				if err != nil {
					t.Fatalf(
						"initializeBuildInnerState returned error: %v",
						err,
					)
				}
			case <-time.After(time.Second):
				t.Fatal(
					"timed out waiting for initializeBuildInnerState to return",
				)
			}
		},
	)
	...
	t.Run(
		"development mode wraps build ID generation error",
		func(t *testing.T) {
			fixture := testkit.NewBuildTestFixture(t, nil)
			app := fixture.App
			...
			if err == nil {
				t.Fatal(
					"expected initializeBuildInnerState to return build ID generation error",
				)
			}
			...
			if !errors.Is(err, expectedErr) {
				t.Fatalf(
					"error = %v, expected wrapped build ID generation error",
					err,
				)
			}
		},
