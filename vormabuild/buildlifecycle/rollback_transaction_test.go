...
func TestRunWithRollbackOnFailureAndPanic(t *testing.T) {
	...
	t.Run("returns run error when rollback succeeds", func(t *testing.T) {
		...
		if err == nil {
			t.Fatal(
				"expected RunWithRollbackOnFailureAndPanic to return run error",
			)
		}
	...
	t.Run(
		"returns run error when rollback step is not configured",
		func(t *testing.T) {
			expectedErr := errors.New("run failed")
			err := RunWithRollbackOnFailureAndPanic(
				RollbackTransactionOptions{
	...
	t.Run(
		"joins run and rollback errors with rollback context",
		func(t *testing.T) {
			...
			if err == nil {
				t.Fatal(
					"expected RunWithRollbackOnFailureAndPanic to return joined error",
				)
			}
			...
			if !errors.Is(err, expectedRollbackErr) {
				t.Fatalf(
					"error = %v, expected rollback error in joined chain",
					err,
				)
			}
	...
	t.Run(
		"logs rollback failure after panic and re-panics",
		func(t *testing.T) {
			expectedPanic := errors.New("run panic")
			expectedRollbackErr := errors.New("rollback failed")
			...
			defer func() {
				...
				if recoveredPanicValue == nil {
					t.Fatal(
						"expected RunWithRollbackOnFailureAndPanic to panic",
					)
				}
				recoveredPanicErr, ok := recoveredPanicValue.(error)
				if !ok {
				...
				if !errors.Is(loggedRollbackErr, expectedRollbackErr) {
					t.Fatalf(
						"logged rollback error = %v, want %v",
						loggedRollbackErr,
						expectedRollbackErr,
					)
				}
	...
	t.Run(
		"panics with rollback panic when run returns error and rollback panics",
		func(t *testing.T) {
	...
	t.Run(
		"preserves original run panic when rollback panics",
		func(t *testing.T) {
			...
			defer func() {
				...
				if !strings.Contains(
					loggedRollbackErr.Error(),
					"rollback panic",
				) {
					t.Fatalf(
						"logged rollback error = %q, expected rollback panic context",
						loggedRollbackErr,
					)
				}
				if !strings.Contains(
					loggedRollbackErr.Error(),
					expectedRollbackPanic.Error(),
				) {
					t.Fatalf(
						"logged rollback error = %q, expected original rollback panic text",
						loggedRollbackErr,
					)
				}
			}()
