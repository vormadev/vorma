	...
	t.Run(
		...
		func(t *testing.T) {
			...
			fixture := newBuildTestFixture(
				t,
				&buildTestFixtureOptions{config: &cfg},
			)
			t.Chdir(fixture.rootDir)
...
func TestBackendRoutePackageAnalysis_GoTypesInitializationIsLazyAndFallbackSafe(
	t *testing.T,
) {
	t.Run(
...
`))
			...
			serverRouteDefinitionFiles, err := resolveServerRouteDefinitionFiles(
				fixture.app,
			)
			if err != nil {
				t.Fatalf(
					"resolveServerRouteDefinitionFiles returned error: %v",
					err,
				)
			}
