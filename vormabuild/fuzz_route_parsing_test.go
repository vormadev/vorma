...
func FuzzExtractRouteCalls_IsDeterministicAndSafe(f *testing.F) {
	f.Add(`import { route } from "vorma/buildtime"; route("/", "./root.tsx", "default");`)
	f.Add(`import { route as defineRoute } from "vorma/buildtime"; const p = "./x.tsx"; defineRoute("/x", p);`)
	f.Add(`import { route } from "vorma/client"; route("/", "./ignored.tsx");`)
	f.Add(`import { route } from "vorma/buildtime"; route("/a", import("./a.tsx"), "A");`)
	...
	f.Fuzz(func(t *testing.T, source string) {
		...
		if errA != nil && errB != nil && errA.Error() != errB.Error() {
			t.Fatalf("error text mismatch: %q vs %q", errA.Error(), errB.Error())
		}
		...
		if !reflect.DeepEqual(unresolvedA, unresolvedB) {
			t.Fatalf("unresolved mismatch: %#v vs %#v", unresolvedA, unresolvedB)
		}

		for _, unresolved := range unresolvedA {
			if strings.TrimSpace(unresolved.Reason) == "" {
				t.Fatalf("unresolved route reason should not be empty: %#v", unresolved)
			}
		}
	})
}
...
func FuzzParseClientRoutes_IsSafeForRouteFileInputs(f *testing.F) {
	f.Add(`import { route } from "vorma/buildtime"; route("/", "./components/root.tsx", "default");`)
	f.Add(`import { route } from "vorma/buildtime"; const m = "./components/root.tsx"; route("/a", m);`)
	f.Add(`import { route } from "vorma/buildtime"; route("/b", import("./components/root.tsx"));`)
	...
	f.Fuzz(func(t *testing.T, routeSource string) {
		...
		mustWriteFile(t, "frontend/src/components/root.tsx", []byte("export default function Root() {}"))
		...
		for pattern, p := range paths {
			...
			if p.OriginalPattern != pattern {
				t.Fatalf("original pattern mismatch: map key=%q value=%q", pattern, p.OriginalPattern)
			}
