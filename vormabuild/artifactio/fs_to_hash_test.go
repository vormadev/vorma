...
func TestGetFSSummaryHash_ChangesWhenFileContentChangesAtSamePathAndSize(
	t *testing.T,
) {
	fsA := fstest.MapFS{
...
func TestGetFSSummaryHash_ChangesWhenSummaryChanges(t *testing.T) {
	t.Run("size change changes hash", func(t *testing.T) {
		...
		if bytes.Equal(beforeHash, afterHash) {
			t.Fatalf(
				"expected different hashes when size changes, got %x",
				beforeHash,
			)
		}
	})
	...
	t.Run("path change changes hash", func(t *testing.T) {
		...
		if bytes.Equal(beforeHash, afterHash) {
			t.Fatalf(
				"expected different hashes when path changes, got %x",
				beforeHash,
			)
		}
	})
