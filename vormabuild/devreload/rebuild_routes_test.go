...
func TestCleanRouteManifestsOnly_RemovesOnlyManifestFiles(t *testing.T) {
	...
	testkit.MustWriteFile(t, manifestFile, []byte("a"))
	testkit.MustWriteFile(t, manifestFile2, []byte("b"))
	testkit.MustWriteFile(t, keepFile, []byte("keep"))
	testkit.MustWriteFile(
		t,
		filepath.Join(manifestDir, "nested.txt"),
		[]byte("nested"),
	)
...
func TestWriteFastRebuildArtifactsAfterRouteSync(t *testing.T) {
	...
	t.Run(
		...
		func(t *testing.T) {
			...
			testkit.MustWriteFile(t, previousManifestPath, []byte(`{"/":0}`))
...
func TestCaptureFastRebuildRouteManifestArtifact(t *testing.T) {
	t.Run(
		...
		func(t *testing.T) {
			...
			if snapshot.Existed() {
				t.Fatalf(
					"snapshot.Existed() = %v, want false",
					snapshot.Existed(),
				)
			}
			if snapshot.Content() != nil {
				t.Fatalf(
					"snapshot.Content() = %#v, want nil",
					snapshot.Content(),
				)
			}
		},
