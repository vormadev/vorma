...
func TestSyncClientRoutesFromParsedPathsWithLock(t *testing.T) {
	...
	t.Run("syncs paths and updates build ID when provided", func(t *testing.T) {
		err := buildlifecycle.SyncClientRoutesFromParsedPathsWithLock(
			app,
		...
		if err != nil {
			t.Fatalf(
				"buildlifecycle.SyncClientRoutesFromParsedPathsWithLock returned error: %v",
				err,
			)
		}
	...
	t.Run(
		"post-sync hook runs without holding runtime write lock",
		func(t *testing.T) {
			err := buildlifecycle.SyncClientRoutesFromParsedPathsWithLock(
				app,
				map[string]*vormaruntime.Path{
				...
				func(*vormaruntime.Vorma) error {
					...
					case <-time.After(100 * time.Millisecond):
						return errors.New(
							"post-sync hook executed while runtime write lock was held",
						)
					}
				},
			)
	...
	t.Run("propagates post-sync hook error", func(t *testing.T) {
			...
			func(v *vormaruntime.Vorma) error {
				...
				v.WithLock(func(l *vormaruntime.LockedVorma) {
					l.SetRouteManifestFile(
						"manifest-set-in-failing-post-sync-hook.json",
					)
					...
					if _, ok := l.Paths()["/about"]; !ok {
						t.Fatalf(
							"expected /about path to be synced before post-sync hook, got %#v",
							l.Paths(),
						)
					}
				})
		...
		if err == nil {
			t.Fatal(
				"expected buildlifecycle.SyncClientRoutesFromParsedPathsWithLock to return hook error",
			)
		}
		...
		if got := app.BuildID(); got != "build-before-failed-post-sync" {
			t.Fatalf(
				"build ID after failed post-sync = %q, want %q",
				got,
				"build-before-failed-post-sync",
			)
		}
		...
		if paths["/existing"] == nil {
			t.Fatalf(
				"expected rollback to restore /existing path, got %#v",
				paths,
			)
		}
		if paths["/about"] != nil {
			t.Fatalf(
				"expected rollback to remove failed /about sync path, got %#v",
				paths["/about"],
			)
		}
	})
	...
	t.Run(
		"skips rollback when runtime state changed after sync but before post-sync error",
		func(t *testing.T) {
			expectedErr := errors.New("post-sync failed")
			app.WithLock(func(l *vormaruntime.LockedVorma) {
				l.SetIsDev(false)
			...
			err := buildlifecycle.SyncClientRoutesFromParsedPathsWithLock(
				...
				func(v *vormaruntime.Vorma) error {
					v.WithLock(func(l *vormaruntime.LockedVorma) {
						...
						l.Routes().
							SyncFromDevReload(map[string]*vormaruntime.Path{
								"/newer": {
			...
			if !app.IsDevMode() {
				t.Fatal(
					"expected newer runtime state to remain after stale rollback attempt",
				)
			}
			if got := app.BuildID(); got != "build-after-newer-sync" {
				t.Fatalf(
					"build ID after stale rollback = %q, want %q",
					got,
			...
			if paths["/newer"] == nil {
				t.Fatalf(
					"expected /newer path to remain after stale rollback, got %#v",
					paths,
				)
			}
			if paths["/existing"] != nil {
				t.Fatalf(
					"did not expect /existing baseline path after stale rollback, got %#v",
					paths["/existing"],
				)
			}
		},
	...
	t.Run(
		"preserves newer runtime state when overlapping route sync attempts race",
		func(t *testing.T) {
			app.WithLock(func(l *vormaruntime.LockedVorma) {
				l.SetIsDev(false)
			...
			go func() {
				firstSyncErrCh <- buildlifecycle.SyncClientRoutesFromParsedPathsWithLock(
					app,
					map[string]*vormaruntime.Path{
						"/first": {
			...
			case <-time.After(time.Second):
				t.Fatal(
					"timed out waiting for first route sync post-sync hook to start",
				)
			}

			if err := buildlifecycle.SyncClientRoutesFromParsedPathsWithLock(
				app,
				map[string]*vormaruntime.Path{
					"/second": {
				...
				t.Fatalf(
					"second overlapping route sync returned error: %v",
					err,
				)
			}
			...
			case err := <-firstSyncErrCh:
				if err == nil {
					t.Fatal(
						"expected first overlapping route sync to return post-sync error",
					)
				}
				if !errors.Is(err, firstSyncExpectedErr) {
					t.Fatalf(
						"first overlapping route sync error = %v, expected wrapped post-sync error",
						err,
					)
				}
			case <-time.After(time.Second):
				t.Fatal(
					"timed out waiting for first overlapping route sync to return",
				)
			}

			if got := app.BuildID(); got != "build-second-sync" {
				t.Fatalf(
					"build ID after overlapping route syncs = %q, want %q",
					got,
					"build-second-sync",
				)
			}
			...
			if paths["/second"] == nil {
				t.Fatalf(
					"expected second-sync path to remain after overlapping route syncs, got %#v",
					paths,
				)
			}
			if paths["/first"] != nil {
				t.Fatalf(
					"did not expect first-sync path after overlapping route syncs, got %#v",
					paths["/first"],
				)
			}
		},
	...
	t.Run(
		"restores runtime state then re-panics when post-sync hook panics",
		func(t *testing.T) {
			app.WithLock(func(l *vormaruntime.LockedVorma) {
			...
			defer func() {
				...
				if recoveredPanicValue == nil {
					t.Fatal(
						"expected buildlifecycle.SyncClientRoutesFromParsedPathsWithLock to panic",
					)
				}
				...
				if paths["/about"] != nil {
					t.Fatalf(
						"expected /about path to be removed after panic rollback, got %#v",
						paths["/about"],
					)
				}
			}()

			_ = buildlifecycle.SyncClientRoutesFromParsedPathsWithLock(
				app,
				map[string]*vormaruntime.Path{
					"/about": {
						OriginalPattern: "/about",
				...
				func(v *vormaruntime.Vorma) error {
					v.WithLock(func(l *vormaruntime.LockedVorma) {
						l.SetRouteManifestFile(
							"manifest-set-in-panicking-post-sync-hook.json",
						)
					})
					panic(expectedPanic)
				},
	...
	t.Run(
		"preserves newer runtime state when post-sync hook panics after newer commit",
		func(t *testing.T) {
			app.WithLock(func(l *vormaruntime.LockedVorma) {
				l.SetIsDev(false)
				l.SetBuildID("build-before-panic-post-sync")
			...
			defer func() {
				...
				if !app.IsDevMode() {
					t.Fatal(
						"expected newer runtime state to remain after stale panic rollback attempt",
					)
				}
				if got := app.BuildID(); got != "build-after-newer-sync" {
					t.Fatalf(
						"build ID after stale panic rollback = %q, want %q",
						got,
						"build-after-newer-sync",
					)
				}
				...
				if paths["/newer"] == nil {
					t.Fatalf(
						"expected /newer path to remain after stale panic rollback, got %#v",
						paths,
					)
				}
				if paths["/existing"] != nil {
					t.Fatalf(
						"did not expect /existing baseline path after stale panic rollback, got %#v",
						paths["/existing"],
					)
				}
			}()
...
func TestPrepareParsedRouteSyncInput(t *testing.T) {
	t.Run(
		"returns parse error when parse context is empty",
		func(t *testing.T) {
			expectedErr := errors.New("parse failed")
			buildIDCalled := false

			_, _, err := buildlifecycle.PrepareParsedRouteSyncInput(
				&vormaruntime.Vorma{},
	...
	t.Run(
		"wraps parse error when parse context is provided",
		func(t *testing.T) {
			expectedErr := errors.New("parse failed")
	...
	t.Run("returns parsed paths with generated build ID", func(t *testing.T) {
		...
		preparedPaths, preparedBuildID, err := buildlifecycle.PrepareParsedRouteSyncInput(
			&vormaruntime.Vorma{},
			func(*vormaruntime.Vorma) (map[string]*vormaruntime.Path, error) {
		...
		if err != nil {
			t.Fatalf(
				"buildlifecycle.PrepareParsedRouteSyncInput returned error: %v",
				err,
			)
		}
	...
	t.Run(
		"returns parsed paths with empty build ID when generator is nil",
		func(t *testing.T) {
			preparedPaths, preparedBuildID, err := buildlifecycle.PrepareParsedRouteSyncInput(
				&vormaruntime.Vorma{},
				func(*vormaruntime.Vorma) (map[string]*vormaruntime.Path, error) {
			...
			if preparedBuildID != "" {
				t.Fatalf(
					"prepared build ID = %q, want empty string",
					preparedBuildID,
				)
			}
		},
...
func TestRunRouteSyncExecution(t *testing.T) {
	t.Run("returns error when parse function is missing", func(t *testing.T) {
		...
		if err == nil {
			t.Fatal(
				"expected buildlifecycle.RunRouteSyncExecution to return error when parse function is missing",
			)
		}
		if !strings.Contains(
			err.Error(),
			"route sync parse function is required",
		) {
	...
	t.Run("wraps parse error using provided parse context", func(t *testing.T) {
		...
		if err == nil {
			t.Fatal(
				"expected buildlifecycle.RunRouteSyncExecution to return parse error",
			)
		}
		if !strings.Contains(err.Error(), "parse client routes") {
			t.Fatalf("error = %q, expected parse context", err)
	...
	t.Run(
		"parses, generates build ID, syncs routes, then runs post-sync hook",
		func(t *testing.T) {
			fixture := newBuildTestFixture(t, nil)
			app := fixture.app
				...
				buildlifecycle.RouteSyncExecutionOptions{
					...
					PostSyncHook: func(v *vormaruntime.Vorma) error {
						...
						v.WithRLock(func(l *vormaruntime.ReadLockedVorma) {
							...
							if l.Paths()["/ok"] == nil {
								t.Fatalf(
									"expected synced /ok route, got %#v",
									l.Paths(),
								)
							}
						})
			...
			if err != nil {
				t.Fatalf(
					"buildlifecycle.RunRouteSyncExecution returned error: %v",
					err,
				)
			}
			if got := strings.Join(observedStepOrder, ","); got != "parse,build-id,post-sync" {
				t.Fatalf(
					"observed step order = %q, want %q",
					got,
					"parse,build-id,post-sync",
				)
			}
		},
	)
