...
function flatten_and_sort(
	...
	if (
		...
		if (pat_seg_len < real_seg_len) return null;
...
export function findNestedMatches(
	...
	for (let i = 0; i < real_segs.length; i++) {
		...
		if (rr) {
			...
			if (i === real_segs_len - 1) found_full_static = true;
		}
	...
	let type_count = 0;
	if (longest_index !== null) type_count++;
	if (longest_dynamic !== null) type_count++;
	if (longest_splat !== null) type_count++;
