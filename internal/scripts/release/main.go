...
const (
	...
	createPackageJSONPath    = "./vormaclient/create/package.json"
)
...
func rollbackPackageJSONVersionsIfUpdated(
	...
	if createPackageJSONWasUpdated {
		...
		if rollbackCreateErr != nil {
			t.Plain("WARNING: failed to rollback vormaclient/create/package.json: ")
			...
			t.Plain("rolled back vormaclient/create/package.json after failure")
			t.NewLine()
		}
	}
}
