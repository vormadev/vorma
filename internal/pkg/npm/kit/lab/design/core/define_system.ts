...
type SemanticRefTree<TSemantic> = TokenRefTree<
	ResolvedTokenTree<
		Exclude<TSemantic, undefined>[keyof Exclude<TSemantic, undefined>]
	>,
...
function create_token_ref_proxy(path: TokenPath): unknown {
	const handler: ProxyHandler<object> = {
		...
		has(_target, property): boolean {
			return (
				property === token_spec_kind ||
				property === token_spec_fields.path
			);
		},
	};
...
function create_style_authoring_helpers<TPrimitive>(
	...
	return {
		...
		textStyle(name) {
			const text_style =
				token.typography[primitive_token_keys.text_style][name]!;
			const typography =
			...
			const letter_spacing =
				typography?.[primitive_token_keys.letter_spacing];
...
>(
	...
	recipe: (
		context: RecipeAuthoringContext<
			TPrimitive,
			NoInferFromUsage<TSemantic>
		>,
	) => TRecipes,
): TRecipes {
	const primitive = create_token_ref_proxy([
