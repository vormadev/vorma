...
export type SliderRecipeInput<TLayout extends string = string> =
...
export type SliderRecipeLayout<TRecipe extends SliderRecipeInput> =
...
const slider_track_webkit_selectors = [
	"&::-webkit-slider-runnable-track",
] as const;
...
> {
	...
	return () => {
		return (props: SliderProps<TLayout, TBreakpoint>): RemixNode => {
			...
			const progress_variable =
				options.progressVariable ?? sliderProgressVariable;
