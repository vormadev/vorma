...
export type CollapsibleRecipeCondition =
...
const collapsible_conditions =
	mergeRecipeConditionSelectors<CollapsibleRecipeCondition>(
		commonConditions,
		{
...
>(
	...
	function Root(
		handle: Handle<
			CollapsibleRootProps<TLayout, TBreakpoint>,
			CollapsibleContext
		>,
		...
		return (
			props: CollapsibleRootProps<TLayout, TBreakpoint>,
		): RemixNode => {
			const {
				at,
				children,
			...
			return createElement(
				...
				createComponentSlotProps({
					...
					props: {
						...root_props,
						[componentStateAttribute]:
							openStateFromBoolean(current_open),
						mix,
					},
				}),
				children,
