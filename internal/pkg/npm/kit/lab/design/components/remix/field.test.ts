...
function create_test_style_system(): FieldStyleSystem<
	"light",
	typeof field_recipe
> {
	return {
...
describe("Remix Field", () => {
	...
	it("wires label, description, error, and control relationships", () => {
		...
		expect(root?.getAttribute(componentAnatomyAttrs.part)).toBe(
			fieldAnatomy.root,
		);
		...
		expect(error?.getAttribute(componentAnatomyAttrs.part)).toBe(
			fieldAnatomy.error,
		);
