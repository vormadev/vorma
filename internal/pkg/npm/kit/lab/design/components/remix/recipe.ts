...
export type RecipeConditionSelectorMap<TCondition extends string = string> =
	Partial<Readonly<Record<TCondition, string>>>;
...
function condition_styles_to_selectors<TCondition extends string>(
	...
	for (const [condition_name, condition_style] of Object.entries(
		condition_styles,
	) as [TCondition, RecipeStyle | undefined][]) {
		if (!condition_style) {
			continue;
		}
