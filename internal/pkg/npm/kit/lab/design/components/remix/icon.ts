...
export type IconRecipeSize<TRecipe extends IconRecipeInput> =
	RecipeVariantValue<TRecipe, "size">;
export type IconRecipeTone<TRecipe extends IconRecipeInput> =
	RecipeVariantValue<TRecipe, "tone">;
export type IconRecipeLayout<TRecipe extends IconRecipeInput> =
	RecipeVariantValue<TRecipe, "layout">;
export type IconRecipeSelection<TRecipe extends IconRecipeInput> =
	RecipeVariantPropsFor<TRecipe, "layout" | "size" | "tone">;
...
> {
	...
	return () => {
		return (
			props: IconProps<TSize, TTone, TLayout, TBreakpoint>,
		): RemixNode => {
			const {
				as = "span",
				at,
