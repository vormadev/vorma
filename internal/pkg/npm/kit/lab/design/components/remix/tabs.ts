...
export type TabsRecipeCondition =
...
export type TabsRecipeLayout<TRecipe extends TabsRecipeInput> =
	RecipeVariantValue<TRecipe, "layout">;

export type TabsRecipeVariant<TRecipe extends TabsRecipeInput> =
	RecipeVariantValue<TRecipe, "variant">;

export type TabsRecipeSize<TRecipe extends TabsRecipeInput> =
	RecipeVariantValue<TRecipe, "size">;

export type TabsRecipeSelection<TRecipe extends TabsRecipeInput> =
...
> {
	...
	function Trigger(
		...
		return (
			props: TabsTriggerProps<TVariant, TSize, TBreakpoint>,
		): RemixNode => {
			const {
				at,
				children,
			...
			return createElement(
				...
				createComponentSlotProps({
					...
					props: {
						...
						[componentStateAttribute]:
							selectionStateFromBoolean(selected),
	...
	function Panel(
		handle: Handle<TabsPanelProps>,
	): (props: TabsPanelProps) => RemixNode {
		const context = handle.context.get(Root);
