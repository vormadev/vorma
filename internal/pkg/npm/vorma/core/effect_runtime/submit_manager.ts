...
export type SubmitManagerOptions = {
	...
	on_snapshot_change?: (snapshot: SubmitSnapshot) => Effect.Effect<void>;
};
...
function submit_error_message(error: unknown): string {
	if (error instanceof SubmitDispatchFailed) {
		return submit_error_message(error.error);
	}
...
export function make_submit_manager(
	...
	return Effect.gen(function* () {
		...
		const report_build_skew =
			options.report_build_skew ??
			((
				_dispatch: SubmitDispatch,
				_response: Response,
				_default_behavior: BuildSkewDetectedEvent["defaultBehavior"],
			): Effect.Effect<boolean> => {
		...
		const model_snapshot = (current: Model): SubmitSnapshot => {
			return {
				...
				active: Array.from(current.active.values(), (submission) => {
					return {
						id: submission.id,
						key: submission.key,
						...
						skipworkIndicator: submission.skipworkIndicator,
					};
				}),
			};
		};
			...
			return Effect.catchAll(
				...
				(error) => {
					return Effect.succeed({
						success: false,
						error: submit_error_message(error),
						response,
						revalidation: Effect.succeed(REVALIDATION_OK),
					});
				},
