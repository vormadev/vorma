...
function meta(
	...
	opts?: { booleanAttributes?: string[]; innerHTML?: string },
): HeadEl {
	return {
		tag: "meta",
		attributesKnownSafe: attrs,
		booleanAttributes: opts?.booleanAttributes,
		dangerousInnerHTML: opts?.innerHTML,
	};
}
...
function style(innerHTML: string): HeadEl {
	return {
		tag: "style",
		attributesKnownSafe: {},
...
describe("rest section", () => {
	...
	it("sets dangerousInnerHTML for style content", () => {
		setup_boundary("meta");
		const { start, end } = setup_boundary("rest");
...
describe("boolean attributes", () => {
	...
	it("handles missing booleanAttributes field gracefully", () => {
		setup_boundary("meta");
		const { start, end } = setup_boundary("rest");

		const el: HeadEl = {
			tag: "script",
			attributesKnownSafe: { src: "/app.js" },
			// booleanAttributes intentionally omitted
		};
	...
	it("handles null booleanAttributes field gracefully", () => {
		setup_boundary("meta");
		const { start, end } = setup_boundary("rest");
