...
describe("navigate", () => {
	...
	it("recovers after network failure and allows subsequent navigation", async () => {
		...
		call(0).resolve(
			route_response({
				MatchedPatterns: ["/recover"],
				LoadersData: [{ recovered: true }],
			}),
		);
		const r2 = await nav;
...
describe("navigate redirects", () => {
	...
	it("follows native browser redirects", async () => {
		...
		call(1).resolve(
			route_response({
				MatchedPatterns: ["/native-target"],
				LoadersData: [{ native: true }],
			}),
		);
		const result = await nav;
	...
	it("chains through multiple soft redirects", async () => {
		...
		call(2).resolve(
			route_response({
				MatchedPatterns: ["/final"],
				LoadersData: [{ final: true }],
			}),
		);
		const result = await nav;
	...
	it("resolves relative redirect targets against request URL", async () => {
		...
		await wait_for(1);
		call(0).resolve(
			redirect_response({ [X_CLIENT_REDIRECT]: "child/final" }),
		);
		await wait_for(2);
	...
	it.each([11, 29, 47, 83, 131])(
		...
		async (seed: number) => {
			...
			non_aborted[0]!.resolve(
				route_response({
					MatchedPatterns: [`/stress-${seed}-last`],
					LoadersData: [{ index: nav_count - 1 }],
				}),
			);
...
describe("history commits", () => {
	it("pushes history on navigate", async () => {
		...
		expect(push_spy.mock.calls[0]![2]).toBe(
	...
	it("replaces history when replace is true", async () => {
		...
		expect(replace_spy.mock.calls.length).toBeGreaterThan(
			initial_replace_count,
		);
	});
	...
	it("propagates replace through redirect chains", async () => {
		...
		expect(push_spy).not.toHaveBeenCalled();
		const replace_calls_after = replace_spy.mock.calls.slice(
			initial_replace_count,
		);
		expect(replace_calls_after.length).toBeGreaterThanOrEqual(1);
		const last_replace =
			replace_calls_after[replace_calls_after.length - 1]!;
